...
class LoadInputsAndTargets(object):
    ...
    def __init__(self, mode='asr',
        ...
        self._loaders = {}
        if mode not in ['asr', 'tts', 'mt', 'mlt-asr']:
            raise ValueError(
    ...
    def __call__(self, batch):
        ...
        if self.mode == 'mlt-asr': 
            y1_feats_dict, y2_feats_dict = OrderedDict(), OrderedDict()
        else:
        ...
        for uttid, info in batch:
            ...
            if self.load_output:
                ...
                for idx, inp in enumerate(info['output']):
                    if self.mode == 'mlt-asr':
                        y1 = np.fromiter(map(int, inp['tokenid'].split()), dtype=np.int64)
                        y2 = np.fromiter(map(int, inp['tokenid_bpe'].split()), dtype=np.int64)
                        y1_feats_dict.setdefault(inp['name'], []).append([y1,y2])
                    else:
                        if 'tokenid' in inp:
        ...
        if (self.mode == 'asr') or (self.mode == 'tts') or (self.mode == 'mt'):
            if self.mode == 'asr':
        ...
        elif self.mode == 'mlt-asr':
            return_batch, uttid_list = self._create_batch_asr(x_feats_dict, y1_feats_dict, uttid_list) 
            if self.preprocessing is not None:
                # Apply pre-processing all input features
                for x_name in return_batch.keys():
                    if x_name.startswith("input"):
