...
def freq_mask(spec, F=30, n_mask=2, replace_with_zero=True, inplace=False):
    ...
    for f in fs:
        ...
        if replace_with_zero:
            cloned[:, f_start:f_end] = 0
        else:
            cloned[:, f_start:f_end] = cloned.mean()
    return cloned
...
def time_mask(spec, T=40, n_mask=2, p=1.0, replace_with_zero=False, inplace=False):
    """freq mask for spec agument

    :param numpy.ndarray spec: (time, freq)
    :param int n_mask: the number of masks
    :param float p: portion of length of spectum, limiting masked duration
    :param bool inplace: overwrite
    :param bool replace_with_zero: pad zero on mask if true else use mean
    """
    if T <= 0:
    ...
    for t in ts:
        t = numpy.minimum(t, int(p*len_spectro))
        # avoids randrange error if values are equal and range is empty
        if t <= 0:
            continue
        if t >= len_spectro:
...
def spec_augment(x, resize_mode="PIL", max_time_warp=80,
                 ...
                 max_time_width=100, n_time_mask=2, time_p=1.0, inplace=True, replace_with_zero=True):
    """spec agument
    ...
    x = time_mask(x, max_time_width, n_time_mask, time_p, inplace=inplace, replace_with_zero=replace_with_zero)
    return x
