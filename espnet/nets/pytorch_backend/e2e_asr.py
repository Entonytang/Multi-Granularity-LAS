...
from espnet.nets.pytorch_backend.rnn.decoders_true_postr import decoder_postrtf_for
from espnet.nets.pytorch_backend.rnn.decoders_pred_postr import decoder_postrpred_for
...
class PenaltyReporter(chainer.Chain):
    ...
    def report(self, loss_penalty, loss_ctc, loss_att, acc, cer_ctc, cer, wer, mtl_loss):
        """Report at every step."""
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def decoder_add_arguments(parser):
        ...
        parser.add_argument('--dec-methods', default='prior', type=str,
                            choices=['prior', 'base', 'postr', 'postrpred',
                                     ],
    ...
    def __init__(self, idim, odim, args):
        ...
        print('[e2e_ar.py] This is for standard code analysis. So please check the code ahead.')
        ...
        # decoder
        if args.dec_methods == 'prior':
            self.dec = decoder_for(args, odim, self.sos, self.eos, self.att, labeldist)
        elif args.dec_methods == 'base':
            self.dec = decoder_base_for(args, odim, self.sos, self.eos, self.att, labeldist)
        elif args.dec_methods == 'postr':
            self.dec = decoder_postrtf_for(args, odim, self.sos, self.eos, self.att, labeldist)
        elif args.dec_methods == 'postrpred':
            self.dec = decoder_postrpred_for(args, odim, self.sos, self.eos, self.att, labeldist)
        else:
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        if self.mtlalpha == 1:
            ...
            if (self.dec_methods == 'postr') or (self.dec_methods == 'postrpred'):
                self.loss_att, acc, penalty = self.dec(hs_pad, hlens, ys_pad, self.divpenaltyweight)
            else:
                self.loss_att, acc, penalty = self.dec(hs_pad, hlens, ys_pad)
        self.acc = acc
        ...
        if loss_data < CTC_LOSS_THRESHOLD and not math.isnan(loss_data):
            self.reporter.report(penalty, loss_ctc_data, loss_att_data, acc, cer_ctc, cer, wer, loss_data)
        else:
