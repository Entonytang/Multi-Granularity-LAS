...
from espnet.nets.pytorch_backend.rnn.encoders import encoder_for, extra_encoder_for
from espnet.nets.pytorch_backend.rnn.decoders import decoder_for
from espnet.nets.pytorch_backend.rnn.decoders_base_mltaskrllm import decoder_multiencoder_mltaskller_for
...
class E2Esm(ASRInterface, torch.nn.Module):
    """E2Esm module.
    ...
    def add_arguments(parser):
        """Add arguments."""
        E2Esm.encoder_add_arguments(parser)
        E2Esm.attention_add_arguments(parser)
        E2Esm.decoder_add_arguments(parser)
        return parser

    @staticmethod
    def encoder_add_arguments(parser):
        """Add arguments for the encoder."""
        group = parser.add_argument_group("E2Esm encoder setting")
        # encoder
    ...
    def attention_add_arguments(parser):
        ...
        group = parser.add_argument_group("E2Esm attention setting")
        # attention
    ...
    def __init__(self, idim, odim, args, args2=None):
        """Construct an E2Esm object.

        :param int idim: dimension of inputs
        :param int odim: dimension of outputs
        :param Namespace args: argument Namespace containing options
        """
        super(E2Esm, self).__init__()
        ...
        self.enc_char = extra_encoder_for('blstm', args_series.eprojs, 1, 
                                            args_series.eunits, args_series.eprojs, [1,1], args_series.dropout_rate)
        self.enc_bpe = extra_encoder_for('blstm', args_series_bpe.eprojs, 1, 
                                            args_series_bpe.eunits, args_series_bpe.eprojs, [1,1], args_series_bpe.dropout_rate)        
        
        # attention and decoder
        if args.dec_methods == 'mltaskbaserllm':
            self.ctc = ctc_for(args, args_series.odim_char) # TJ 2019.12.30: CTC module         
            print('For attention : this uses the CHAR-level CTC Auxilitary Loss (mltaskbaserllm)----TRY.')
            print('Function decoder_multiencoder_mltaskller_for().')
            self.att_char, self.att_bpe, self.att_char2bpe = att_for(args_series), att_for(args_series_bpe), att_for(args_series_bpe)
            self.dec = decoder_multiencoder_mltaskller_for(
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        # additional operation for char
        hs_pad_char, hlens_char, _ = self.enc_char(hs_pad, hlens) # char 
        # additional operation for BPE
        hs_pad_bpe, hlens_bpe, _ = self.enc_bpe(hs_pad, hlens)
        hs_pad = [hs_pad_char, hs_pad_bpe]
        hlens = [hlens_char, hlens_bpe]        
        ...
        if self.mtlalpha == 0:
            ...
            if self.dec_methods[-1] == "b":
                loss_ctc = self.ctc(hs_pad_bpe, hlens_bpe, ys_pad2) ## this is BPE CTC loss
            else:
                loss_ctc = self.ctc(hs_pad_char, hlens_char, ys_pad1) ## CHAR
        ...
        if self.mtlalpha == 0:
            ...
            # calculate CTC CER
            if self.dec_methods[-1] == "b":
                y_hats = self.ctc.argmax(hs_pad_bpe).data # [batchsize,decT]
            else:
                y_hats = self.ctc.argmax(hs_pad_char).data # [batchsize,decT]
        ...
        if self.training or not (self.report_cer or self.report_wer): # default setting
            ...
            if self.recog_args.ctc_weight > 0.0:
                if self.dec_methods[-1] == "b":
                    lpz = self.ctc.log_softmax(hs_pad_bpe).data
                else:
                    lpz = self.ctc.log_softmax(hs_pad_char).data
            else:
                lpz = None
    ...
    def recognize_batch(self, xs, recog_args, bpe_list, rnnlm=None):
        ...
        # 2. Decoder
        hlens_bpe = torch.tensor(list(map(int, hlens_bpe)))  # make sure hlens is tensor
        hlens_char = torch.tensor(list(map(int, hlens_char)))  # make sure hlens is tensor
        hlens = [hlens_bpe, hlens_char]
    ...
    def calculate_all_attentions(self, xs_pad, ilens, ys_pad):
        ...
        with torch.no_grad():
            ...
            # additional operation for char
            hs_pad_char, hlens_char, _ = self.enc_char(hpad, hlens) # char        
            # additional operation for BPE 
            hs_pad_bpe, hlens_bpe, _ = self.enc_bpe(hpad, hlens)
            ## concat
            hpad = [hs_pad_bpe, hs_pad_char]
            hlens = [hlens_bpe, hlens_char]
