...
from espnet.nets.pytorch_backend.rnn.decoders_base_entropy import decoder_base_entropy_for
...
class Reporter(chainer.Chain):
    ...
    def report(self, loss_ctc, loss_att, acc, alpe_bpe, cer_ctc, cer, wer, mtl_loss):
...
class PenaltyReporter(chainer.Chain):
    """A chainer reporter wrapper."""

    def report(self, loss_penalty, loss_ctc, loss_att, acc, alpe_bpe, cer_ctc, cer, wer, mtl_loss):
        """Report at every step."""
        reporter.report({'penalty': loss_penalty}, self)
        reporter.report({'loss_ctc': loss_ctc}, self)
        reporter.report({'loss_att': loss_att}, self)
        reporter.report({'acc': acc}, self)
        reporter.report({'alpe_bpe': alpe_bpe}, self)
...
class E2EA(ASRInterface, torch.nn.Module):
    """E2EA module.
    ...
    def add_arguments(parser):
        """Add arguments."""
        E2EA.encoder_add_arguments(parser)
        E2EA.attention_add_arguments(parser)
        E2EA.decoder_add_arguments(parser)
        return parser

    @staticmethod
    def encoder_add_arguments(parser):
        """Add arguments for the encoder."""
        group = parser.add_argument_group("E2EA encoder setting")
        # encoder
    ...
    def attention_add_arguments(parser):
        ...
        group = parser.add_argument_group("E2EA attention setting")
        # attention
    ...
    def decoder_add_arguments(parser):
        ...
        parser.add_argument('--dec-methods', default='prior', type=str,
                            choices=[
                                        'base'
                                     ],
                            help='Choose Decoder Type') ## added by jtang ##     
        parser.add_argument('--dec_filts', default=3, type=int,
                            help='Number of filter-size for convolution block (2*dec_filts+1)')


        return parser

    def __init__(self, idim, odim, args):
        """Construct an E2EA object.

                                                                                        :param int idim: dimension of inputs
        :param int odim: dimension of outputs
        :param Namespace args: argument Namespace containing options
        """
        super(E2EA, self).__init__()
        torch.nn.Module.__init__(self)
        self.mtlalpha = args.mtlalpha
        assert 0.0 <= self.mtlalpha <= 1.0, "mtlalpha should be [0.0, 1.0]"
        print('[E2EA_ar.py] This is for standard code analysis. So please check the code ahead.')
        ...
        print("The penalty weight here is %f" %(args.divpenaltyweight))
        self.reporter = PenaltyReporter()
        self.divpenaltyweight = args.divpenaltyweight
        
        #if (args.dec_methods == 'postr') or (args.dec_methods == 'postrpred'):
        #    print("Using PenaltyReporter in report (dec_methods is postr/postrpred).")
        #    print("The penalty weight here is %f" %(args.divpenaltyweight))
        #    self.reporter = PenaltyReporter()
        #    self.divpenaltyweight = args.divpenaltyweight
        #else:
        #    print("Using the default Reporter in report.")
        #    self.reporter = Reporter()
        
        self.dec_methods = args.dec_methods ## added by jtang ##  
        ...
        # decoder
        if args.dec_methods == 'base':
            print('base setting.')
            self.dec = decoder_base_entropy_for(args, odim, self.sos, self.eos, self.att, labeldist)
        else:
            print('Error setting for e2e_asr.py')
            exit(0)
    ...
    def init_like_chainer(self):
        ...
        if (not self.dec_methods == 'base') and (not self.dec_methods == 'postr') and (not self.dec_methods == 'postrpred'):
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        if self.mtlalpha == 1:
            ...
            if (self.dec_methods == 'postr') or (self.dec_methods == 'postrpred'):
                self.loss_att, acc, penalty, alpe_bpe = self.dec(hs_pad, hlens, ys_pad, self.divpenaltyweight)
            else:
                self.loss_att, acc, penalty, alpe_bpe = self.dec(hs_pad, hlens, ys_pad)
        self.acc = acc
        ...
        if loss_data < CTC_LOSS_THRESHOLD and not math.isnan(loss_data):
            self.reporter.report(penalty, loss_ctc_data, loss_att_data, acc, float(alpe_bpe), cer_ctc, cer, wer, loss_data)
        else:
