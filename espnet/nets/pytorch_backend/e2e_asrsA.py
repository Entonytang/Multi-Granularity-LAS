...
from espnet.nets.pytorch_backend.rnn.decoders_base_mltaskrll_modif3aux2 import decoder_base_mltaskller_modification3_aux2_for
from espnet.nets.pytorch_backend.rnn.decoders_base_mltaskrll_modif3aux6 import decoder_base_mltaskller_modification3_aux6_for
CTC_LOSS_THRESHOLD = 10000

class MltReporter(chainer.Chain):
    """A chainer reporter wrapper"""

    def report(self, loss_ctc, loss_att_char, loss_att_bpe, loss_aux, penalty, acc_char, acc_bpe, acc_aux, cer_ctc, cer, wer, mtl_loss):
...
class E2EsA(ASRInterface, torch.nn.Module):
    """E2EsA module.
    ...
    def add_arguments(parser):
        """Add arguments."""
        E2EsA.encoder_add_arguments(parser)
        E2EsA.attention_add_arguments(parser)
        E2EsA.decoder_add_arguments(parser)
        return parser

    @staticmethod
    def encoder_add_arguments(parser):
        """Add arguments for the encoder."""
        group = parser.add_argument_group("E2EsA encoder setting")
        # encoder
    ...
    def attention_add_arguments(parser):
        ...
        group = parser.add_argument_group("E2EsA attention setting")
        # attention
    ...
    def __init__(self, idim, odim, args, args2=None):
        """Construct an E2EsA object.

        :param int idim: dimension of inputs
        :param int odim: dimension of outputs
        :param Namespace args: argument Namespace containing options
        """
        super(E2EsA, self).__init__()
        ...
        if args.dec_methods == 'mltaskbaserll_modi3a2':
            ...
            print('[MultiAUX] decoder_base_mltaskller_modification3_for')
            print('mltaskbaserll_modi3a2')
            self.att_char, self.att_bpe, self.att_char2bpe = att_for(args_series), att_for(args_series_bpe), att_for(args_series_bpe)
            self.dec = decoder_base_mltaskller_modification3_aux2_for(
        ...
        elif args.dec_methods == 'mltaskbaserll_modi3a6':
            self.ctc = ctc_for(args, args_series.odim_char) # TJ 2019.12.30: CTC module         
            print('For attention : this uses the CHAR-level CTC Auxilitary Loss (mltaskbaserll_modi3a6).')
            print('[MultiAUX] decoder_base_mltaskller_modification3_for')
            self.att_char, self.att_bpe, self.att_char2bpe = att_for(args_series), att_for(args_series_bpe), att_for(args_series_bpe)
            self.dec = decoder_base_mltaskller_modification3_aux6_for(
                            args_series, args_series.odim_char, args_series.odim_bpe,
                            self.sos_char, self.eos_char, self.sos_bpe, self.eos_bpe, self.att_char, self.att_bpe, self.att_char2bpe, labeldist_char, labeldist_bpe
                        )            
        else:
            print("[e2e_asrsA.py-257] This decoder methods is not suitable for e2e_asrs.py (%s)." %(args.dec_methods))
            exit(0)
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        # 3. attention loss
        [loss_att, aux_loss_att, bpeaux1], [acc_char, acc_bpe, acc_bpeaux1] = self.dec(hs_pad, hlens, ys_pad1, ys_pad2)
        ...
        if loss_data < CTC_LOSS_THRESHOLD and not math.isnan(loss_data):
            self.reporter.report(loss_ctc_data, char_loss_att_data, bpe_loss_att_data, bpeaux1_data, penalty_data, acc1, acc2, acc3, cer_ctc, cer, wer, loss_data) ## acc3
        else:
        ...
        return self.loss, loss_ctc, char_loss_att_data+bpe_loss_att_data, acc1, acc2, acc3, cer, wer # char_loss_att_data is float()
