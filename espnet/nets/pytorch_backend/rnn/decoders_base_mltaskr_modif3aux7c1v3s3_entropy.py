...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('[decoder_base_mltaskller_modification3_for] This is info transformation from char to BPE (mltaskr_modi3a7c1v3 -- h2->bpe class ).')
        print('就是c1v3 -- 将RNN3处的ctx和hinter_都关闭')
        print('-----------------------------------------------------------------------------------------------------------------')
        ...
        # self.char2bpe_rnn1_W1 = torch.nn.Linear(dunits, dunits*2, bias=False)        
        # self.char2bpe_rnn1_W2 = torch.nn.Linear(dunits, dunits*2, bias=False)

        # self.char2bpe_rnn1_Wx1= torch.nn.Linear(dunits, dunits, bias=False)
        # self.char2bpe_rnn1_Wx2= torch.nn.Linear(dunits, dunits, bias=False)
        ...
        # self.classbpe_yprev_W = torch.nn.Linear(self.embsize_bpe, odim_bpe, bias=False)
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        if SWITCH_BPE == True:
            ...
            for i in six.moves.range(olength_bpe):
                ...
                aux2_att_state.append(h2)

                ## 1. RNN : s'(t)=RNN(y(t-1),s(t-1)) 
                # || to combine [emb(t-1),state(t-1)] -> predt-state(t) 
                # || RNN(dunits) : concat[ey,ctx-previous-posterior]
                preact3 = self.char2bpe_rnn1_U(hbpe_) # no bias -> [B,dunits*2]=[10,600]
                # preact3 += self.char2bpe_rnn1_W1(char_ctx_)
                # preact3 += self.char2bpe_rnn1_W2(inter_h_)
                ...
                preactx3 *= r3
                preactx3 += c2b_xx_
                # preactx3 += self.char2bpe_rnn1_Wx1(char_ctx_)
                # preactx3 += self.char2bpe_rnn1_Wx2(inter_h_)                 
                ...
                ## 3. RNN : s(t)=RNN(c(t),s'(t)) 
                # || att_c(eprojs) z_infocomb_list(dunits)
                h13comb = torch.sigmoid( self.c2b_statecomb(h3) ) + h1
                hctxcomb = torch.sigmoid( self.c2b_ctxcomb(c2b_prior_att_c) ) + prior_att_c
                ...
                logit_inter = self.classbpe_state_W(h4) + self.classbpe_enc_W(c2b_prior_att_c) # [B,dunits]=[10,300]
                probs = F.softmax(logit_inter, dim=-1) # [B,496]
        ...
        bpe_att_state = torch.stack(aux2_att_state, dim=1)
        ypred_logit_bpe = (self.FCaux2_class_state(bpe_att_state) + self.FCaux2_class_context(bpe_att_context))
        ypred_logit_bpe = ypred_logit_bpe.view(batch * olength_bpe, self.odim_bpe)
        ## 8.3 : bpe loss
        ypred_bpe = F.softmax(ypred_logit_bpe, dim=-1)
    ...
    def char_onebpe_training(self, exp_h, exp_hlens, prev_alpha, h_, yseq, char_len, char_output_all, charposterindex, char_ctx_, strm_idx=0):  # realmask
        ...
        for ii in range(int(max(char_len))):
            ...
            logit_char = self.classchar_state_W(h2) + self.classchar_enc_W(prior_att_c) # [nbatch,dunits]
            probs = F.softmax(logit_char, dim=-1)                                                                                     # [nbatch,odim]
    ...
    def calculate_all_attentions(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0):  #### change here
        ...
        if SWITCH_BPE == True:
            ...
            for i in six.moves.range(olength_bpe):
                ...
                if decode_choice == "bpe":
                    ...
                    subprev_alpha, hbpe_, char_output_all, charposterindex, char_ctx_ = self.char_onebpe_training(
                        bpe_enc, hlens_bpe, subprev_alpha, hbpe_, ysequence, char_len[i,:], 
                        char_output_all, charposterindex, char_ctx_) # char_len[i,:] is batch
        ...
        # ys_char = [y[y != self.ignore_id] for y in ys_pad_char]
        # ys_bpe = [y[y != self.ignore_id] for y in ys_pad_bpe]
        # hlens = hlens.cpu().detach().numpy() # numpy-(batchsize)
        # att_ws_bpe = att_to_numpy(bpe_prior_att_weights, self.att_bpe[att_bpe_idx]) 
        # att_ws_c2b = att_to_numpy(c2b_prior_att_weights, self.att_bpe[att_bpe_idx]) 
        # return (att_ws_bpe, att_ws_c2b, ys_char, ys_bpe, hlens, ys_char_out_pad.cpu().detach().numpy() ) # 
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        nbest_hyps = [sorted(ended_hyps[samp_i], key=lambda x: x['score'], reverse=True)[:min(len(ended_hyps[samp_i]), recog_args.nbest)] # recog_args.nbest (1)
                      for samp_i in six.moves.range(batch)]
        print('[mltaskraux7c1v3s1] The value of CHOICE_SCORE_TYPES is %s. TJ-USING.' %(CHOICE_SCORE_TYPES))
...
def decoder_base_mltaskr_modification3_aux7c1v3s3_entropy_for(
