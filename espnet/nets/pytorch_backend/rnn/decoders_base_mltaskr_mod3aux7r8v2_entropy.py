...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('+ v2 : 基于r8r8 小改动：对于RNN3处的info-GLU操作a+sig(a)*b 进行融合，RNN4保持不变！')
        print('-----------------------------------------------------------------------------------------------------------------')
        ## Deep Fusion Like FC 
        ...
        if SWITCH_BPE == True:
            ...
            for i in six.moves.range(olength_bpe):
                ...
                if auxilitary_char2bpe_inter_part == True:
                    ...
                    info = torch.sigmoid( self.c2b_statecomb2(hbpe_) )*inter_h_ + hbpe_
                    ...
                    h3 = u3 * info + (1. - u3) * h3
                    h3 = m_.unsqueeze(1) * h3 + (1. - m_).unsqueeze(1) * info
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        for i in six.moves.range(maxlen):
            ...
            inter_h_ = torch.index_select(h4.view(n_bb, *h4.shape[1:]), 0, vidx)
        ...
        nbest_hyps = [sorted(ended_hyps[samp_i], key=lambda x: x['score'], reverse=True)[:min(len(ended_hyps[samp_i]), recog_args.nbest)] # recog_args.nbest (1)
                      for samp_i in six.moves.range(batch)]
        print('[mltaskraux7r8v2] The value of CHOICE_SCORE_TYPES is %s. TJ-USING.' %(CHOICE_SCORE_TYPES))
...
def decoder_base_mltasker_mod3_aux7r8v2_entropy_for(
