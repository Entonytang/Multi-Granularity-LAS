...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        if switch_addcode_forbi == True:
            ...
            self.classb2c_state_W = torch.nn.Linear(dunits, dunits, bias=False)                
        ...
        if switch_addcode_forbi == True: # False is same as mltaskrll / True is BIDIR (mltaskrll + d)
            print('[decoders_base_mltaskrlld.py] This is just information transformation from char to BPE (mltaskbaserllcf).')
            print('The decoder version is multi-level char&bpe (BIDIR : code checking -> second joint decoding).')
            print('b2c-state 先接一个RNN 再接FC 连到classification')
        else:
    ...
    def char_onebpe_training(self, exp_h, exp_hlens, prev_alpha, h_, yseq, char_len, char_output_all, charposterindex, bpe2char_state=None, strm_idx=0):  # realmask
        ...
        for ii in range(int(max(char_len))):
            ...
            if switch_addcode_forbi == False: 
                ...
                preact0 = self.char_rnn0_U(bpe2char_state) # [nbatch,dunits*2]
                preact0 += self.char_rnn0_W(ey) # [nbatch,dunits*2]
                ...
                preactx0 = self.char_rnn0_Ux(bpe2char_state) # [nbatch,dunits]
                preactx0 *= r0
                preactx0 += self.char_rnn0_Wx(ey) 
                h0 = torch.tanh(preactx0)
                h0 = u0 * bpe2char_state + (1. - u0) * h0
                bpe2char_state = m_.unsqueeze(1) * h0 + (1. - m_).unsqueeze(1) * bpe2char_state

                c2b_state = self.classb2c_state_W(bpe2char_state)
...
def decoder_base_mltaskllercf_for(
