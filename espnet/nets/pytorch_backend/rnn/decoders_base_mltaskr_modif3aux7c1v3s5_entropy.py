...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('[decoder_base_mltaskller_modification3_for] This is info transformation from char to BPE (mltaskr_modi3a7c1v3s5 -- h2->bpe class ).')
        print('mltaskr_modi3a7c1v3s5 : 7 aux file + entropy [y(t-1),h4,c2bctx]')
        print('-----------------------------------------------------------------------------------------------------------------')
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        if SWITCH_BPE == True:
            ...
            for i in six.moves.range(olength_bpe):
                ...
                logit_inter = self.classbpe_state_W(h4) + self.classbpe_enc_W(c2b_prior_att_c) + self.classbpe_yprev_W(state_bpe[:, i, :]) # [B,dunits]=[10,300]
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        for i in six.moves.range(maxlen):
            if decode_choice == 'char':
                ...
                ## 4. Classification for every frames
                # h2[B,dunits]  state_bpe[B,decT]  prior_att_c[B,dunits//2]
                logit_inter = self.classbpe_state_W(h4) + self.classbpe_enc_W(c2b_prior_att_c) + self.classbpe_yprev_W(ey) # [B,dunits]=[10,300]
                probs = F.softmax(logit_inter, dim=-1) # [B,496]
        ...
        nbest_hyps = [sorted(ended_hyps[samp_i], key=lambda x: x['score'], reverse=True)[:min(len(ended_hyps[samp_i]), recog_args.nbest)] # recog_args.nbest (1)
                      for samp_i in six.moves.range(batch)]
        print('[mltaskraux7c1v3s5] The value of CHOICE_SCORE_TYPES is %s. TJ-USING.' %(CHOICE_SCORE_TYPES))
...
def decoder_base_mltaskr_modification3_aux7c1v3s5_entropy_for(
