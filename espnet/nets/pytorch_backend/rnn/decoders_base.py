...
class SoftmaxDecoder(torch.nn.Module):
    ...
    def forward(self, hs_pad, hlens, ys_pad, penalty_weight=0, strm_idx=0): 
        ...
        return self.loss, acc, torch.zeros(self.loss.size())
    ...
    def calculate_all_attentions(self, hs_pad, hlens, ys_pad, strm_idx=0):  #### change here
        ...
        # att_ws = torch.stack(prior_att_weights, dim=1).detach().cpu().numpy()
        # import scipy.io as scio
        # test_result = {}
        # test_result['att_ws_bpe'] = att_ws
        # scio.savemat('alpha_ws_bpe.mat', test_result)
        # print('This is for get result.40-BPE')
        # exit(0)
...
def decoder_base_for(args, odim, sos, eos, att, labeldist):
