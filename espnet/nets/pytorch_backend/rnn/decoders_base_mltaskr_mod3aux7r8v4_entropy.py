...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('v4: RNN3(没有GLU辅助输入)额外添加上char_ctx_同时作为辅助输入，在原来GLU RNN3输入融合的基础上。')
        print('-----------------------------------------------------------------------------------------------------------------')
        ## Deep Fusion Like FC 
        ...
        if SWITCH_BPE == True:
            ...
            for i in six.moves.range(olength_bpe):
                ...
                if auxilitary_char2bpe_inter_part == True:
                    ...
                    h13comb = torch.cat([h1, h1h3comb], dim=1)
                    ############
                    ctxcomb = torch.sigmoid( self.c2b_ctxcomb(prior_att_c) ) * c2b_prior_att_c
                    hctxcomb = torch.cat([prior_att_c, ctxcomb], dim=1)
                    ############
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        nbest_hyps = [sorted(ended_hyps[samp_i], key=lambda x: x['score'], reverse=True)[:min(len(ended_hyps[samp_i]), recog_args.nbest)] # recog_args.nbest (1)
                      for samp_i in six.moves.range(batch)]
        print('[mltaskraux7r8v4] The value of CHOICE_SCORE_TYPES is %s. TJ-USING.' %(CHOICE_SCORE_TYPES))
...
def decoder_base_mltasker_mod3_aux7r8v4_entropy_for(
