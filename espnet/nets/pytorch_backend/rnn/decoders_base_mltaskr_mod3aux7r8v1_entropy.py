...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        self.char2bpe_rnn1_U  = torch.nn.Linear(2*dunits, dunits*2, bias=False)
        self.char2bpe_rnn1_Ux = torch.nn.Linear(2*dunits, dunits, bias=False)
        ...
        if SWITCH_BPE == True:
            ...
            for i in six.moves.range(olength_bpe):
                ...
                if auxilitary_char2bpe_inter_part == True:
                    ...
                    infocomb = torch.sigmoid( self.c2b_statecomb2(hbpe_) ) * inter_h_
                    infocomb = torch.cat([hbpe_, infocomb], dim=1)

                    ## 1. RNN : s'(t)=RNN(y(t-1),s(t-1)) 
                    # || to combine [emb(t-1),state(t-1)] -> predt-state(t) 
                    # || RNN(dunits) : concat[ey,ctx-previous-posterior]
                    preact3 = self.char2bpe_rnn1_U(infocomb) # no bias -> [B,dunits*2]=[10,600]
                    ...
                    preactx3 = self.char2bpe_rnn1_Ux(infocomb) # no bias [B,dunits]=[10,300]
                    preactx3 *= r3
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        nbest_hyps = [sorted(ended_hyps[samp_i], key=lambda x: x['score'], reverse=True)[:min(len(ended_hyps[samp_i]), recog_args.nbest)] # recog_args.nbest (1)
                      for samp_i in six.moves.range(batch)]
        print('[mltaskraux7] The value of CHOICE_SCORE_TYPES is %s. TJ-USING.' %(CHOICE_SCORE_TYPES))
...
def decoder_base_mltasker_mod3_aux7r8v1_entropy_for(
