...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('这个版本是：让BPE来产生种子，之后相对应的调整char的----char不会完全随机: espresso SS')
        print('------------------------------------------------------------------------------------------------------------------------------------------------')
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, sampling_probability, strm_idx=0): 
        ...
        if SWITCH_BPE == True:
            ...
            for i in six.moves.range(olength_bpe):
                ...
                if i > 0:
                    sampling_mask = torch.rand([nbatch], device=hs_pad.device,).lt(1.0 - sampling_probability)
                    ypred_argmax  = torch.argmax(probs, dim=1)
                    bpe_prev_ey   = torch.where(sampling_mask, ys_bpe_in_pad[:, i], ypred_argmax,) # [B,1]
                    # pre-computation of embedding y -> emb(y) [B,C,D_senone]
                    ypred_embs = self.dropout_emb_bpe(self.embed_bpe(bpe_prev_ey))  # utt x olen x zdim [B,decoder_T,300]
                    x_         = self.bpe_rnn1_W(ypred_embs)
                    xx_        = self.bpe_rnn1_Wx(ypred_embs)
                    c2b_x_     = self.char2bpe_rnn1_W(ypred_embs)
                    c2b_xx_    = self.char2bpe_rnn1_Wx(ypred_embs)
                else:
                    ypred_embs = self.dropout_emb_bpe(self.embed_bpe(ys_bpe_in_pad[:, i]))
                    x_         = self.bpe_rnn1_W(ypred_embs)  # [B,decT,dunits]=[10,144,600]
                    xx_        = self.bpe_rnn1_Wx(ypred_embs) # [B,decT,dunits]=[10,144,300]
                    c2b_x_     = self.char2bpe_rnn1_W(ypred_embs)  # [B,decT,dunits]=[10,144,600]
                    c2b_xx_    = self.char2bpe_rnn1_Wx(ypred_embs) # [B,decT,dunits]=[10,144,300]                       
                ...
                logit_all_for_classif = self.classbpe_state_W(cstates) + self.classbpe_yprev_W(ypred_embs) + self.classbpe_enc_W(context) # [B,dunits]=[10,300]
...
def decoder_base_mltaskller_m3_schedulesampling3_for(
