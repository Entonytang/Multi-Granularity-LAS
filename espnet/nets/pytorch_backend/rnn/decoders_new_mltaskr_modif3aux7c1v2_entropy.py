...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        self.classchar_state_W = torch.nn.Linear(dunits, odim_char)
        self.classchar_enc_W   = torch.nn.Linear(eprojs, odim_char, bias=False)
        ...
        print('mltaskr_modi3a7c1v2n : 7 aux file + entropy (new)')
        print('-----------------------------------------------------------------------------------------------------------------')
        ...
        self.char2bpe_rnn2_U_n1  = torch.nn.Linear(2*dunits, dunits*2)
        self.char2bpe_rnn2_Wc    = torch.nn.Linear(cprojs, dunits*2, bias=False)
        self.char2bpe_rnn2_Ux_n1 = torch.nn.Linear(2*dunits, dunits)
        ...
        self.classbpe_state_W = torch.nn.Linear(dunits, odim_bpe)
        # self.classbpe_yprev_W = torch.nn.Linear(self.embsize_bpe, dunits, bias=False)
        self.classbpe_enc_W   = torch.nn.Linear(cprojs, odim_bpe, bias=False)
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        if SWITCH_BPE == True: ## 在这一组中采用index 来挑选所需要使用的(t-1) char state 
            ...
            for i in six.moves.range(olength_bpe):
                ...
                aux2_att_state.append(bpe_h2)

                ## 4. Do classification for every frame
                logit_bpe = self.inter_class_state(bpe_h2) + self.inter_class_context(bpe_prior_att_c) # [nbatch,dunits]
                probs = F.softmax(logit_bpe, dim=-1)  
                ...
                if PREPARE_USING_B2C == True:
                    ...
                    logit_inter = self.classbpe_state_W(c2b_h4) + self.classbpe_enc_W(c2b_att_c) # [nbatch,dunits]
                    probs = F.softmax(logit_inter, dim=-1)  
                    inter_output_all.append(probs)
                    ############################## this is interaction operation         
        ...
        ## 8.3 : inter
        ypred_inter = torch.stack(inter_output_all, dim=1).view(batch * olength_bpe, self.odim_bpe)   # [B,decT,odim_bpe]
        self.loss_inter = F.categorical_crossentropy(ypred_inter, ys_bpe_out_pad.contiguous().view(-1).long(), ignore_index=self.ignore_id, reduction='none')
        self.loss_inter = self.loss_inter.view(batch, olength_bpe) 
        self.loss_inter = self.loss_inter.sum(1)
        self.loss_inter = self.loss_inter.mean(0) # to be scalar   
        ...
        self.batchcount += 1
        return [self.loss_char, self.loss_inter, self.loss_bpe], [acc_char, acc_inter, acc_bpe], [ent_bpe, ent_c2b]
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        for i in six.moves.range(maxlen):
            if decode_choice == 'char':
                ...
                ## 4. Do classification for every frame
                logit_inter = self.classbpe_state_W(h4) + self.classbpe_enc_W(c2b_prior_att_c) # [n_bb,dunits]
                probs = F.softmax(logit_inter, dim=1) # [n_bb,odim]
        ...
        nbest_hyps = [sorted(ended_hyps[samp_i], key=lambda x: x['score'], reverse=True)[:min(len(ended_hyps[samp_i]), recog_args.nbest)] # recog_args.nbest (1)
                      for samp_i in six.moves.range(batch)]
        print('[c1v2_entropy] The value of CHOICE_SCORE_TYPES is %s. TJ-USING.' %(CHOICE_SCORE_TYPES))

        return nbest_hyps



def decoder_new_mltaskr_modification3_aux7c1v2_entropy_for(
