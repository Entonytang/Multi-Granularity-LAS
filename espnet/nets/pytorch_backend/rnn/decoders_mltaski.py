...
JOINT_CHAR_DECODER = True
BPE_JointDecoding = True  ### SWITCH
decode_choice = "bpe"
...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('[decoders_mltaski.py] This is second baseline [this code will make some performance decrease] (mtaski).')
        print('Just Know why. No interaction here.')
    ...
    def char_onebpe_training(self, exp_h, exp_hlens, prev_alpha, h_, yseq, char_len, char_output_all, charposterindex, charpredall, strm_idx=0):  # realmask查验
        ...
        bpelen = int(max(char_len))

        for ii in range(bpelen):
        ...
        # 之后再更新charposterindex
        charposterindex += char_len
        charpredall.append(output_all)

        return prev_alpha, h_, char_output_all, charposterindex, charpredall
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        if SWITCH_BPE == True:
            ...
            ## prepare the target result for loss calculation srart  ##
            charlabel_paddingall = []
            for i in six.moves.range(olength_bpe):
                max_len = int(max(char_len[i,:])) 
                sublabel = self.ignore_id * np.ones((nbatch, int(max_len+1) ), dtype=np.int) # (10,5)            
                for subidx,chartoken in enumerate(yseq_char_inbpe[i]):
                    sublabel[subidx,:len(chartoken)] = np.array(chartoken)
                charlabel_paddingall.append(sublabel[:,1:])
            charpredall = []      
            ## prepare the target result for loss calculation finish ##
            ...
            for i in six.moves.range(olength_bpe):
                ...
                if JOINT_CHAR_DECODER == True:
                    ...
                    sub_prior_att_w, subh2, char_output_all, charposterindex, charpredall = self.char_onebpe_training(
                        bpe_enc, hlens_bpe, subprev_alpha, subh_, ysequence, char_len[i,:], char_output_all, charposterindex, charpredall) # char_len[i,:] is batch
                ...
                subh_ = subh2
                subprev_alpha = sub_prior_att_w    
                
            charpredall = torch.cat(charpredall, dim=0).transpose(1, 0).contiguous().view(-1, self.odim_char)            
            charlabelall = torch.Tensor(np.hstack(charlabel_paddingall)).to(hs_pad) ## 为后面Loss计算使用的备用loss情况                            
        ...
        if traditional_loss_calculate == True:
            ...
            self.loss_char = F.categorical_crossentropy(charpredall, charlabelall.contiguous().view(-1).long(), ignore_index=self.ignore_id, reduction='none') # [B,T]
            self.loss_char = self.loss_char.view(batch, -1) 
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        if (decode_choice == 'char'): # extra initialization        
            ...
            # mask all 1 initials
        elif (decode_choice == 'bpe'): # extra initialization
