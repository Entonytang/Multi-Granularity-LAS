...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('[decoders_base_mltaskrll-modification3.py] This is just information transformation from char to BPE (mltaskbaserll-modification3).')
        print('-----------------------------------------------------------------------------------------------------------------')
    ...
    # Forward Process
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, sampling_probability=0.0, strm_idx=0): 
        """Decoder forward
        ...
        if SWITCH_BPE == True:
            ...
            if ( (self.batchcount%5000)==0 ): print('Using sampling_probability (char&bpe joint mask) with %2.1f' %(sampling_probability) )
            ...
            for i in six.moves.range(olength_bpe):
                ...
                if i > 0:
                    ...
                    sampling_mask = torch.ones([nbatch], device=hs_pad.device, dtype=torch.uint8)
                ...
                prior_att_c, prior_att_w = self.att_bpe[att_bpe_idx](bpe_enc, hlens_bpe, h1, prev_alpha)
                ...
                ## 2. Attention : c(t)=AttentionContext(s'(t), enc_o) 
                # || dunits (prior_att_c[B,eprojs] prior_att_w[B,T])
                c2b_prior_att_c, c2b_prior_att_w = self.att_char2bpe[att_bpe_idx](bpe_enc, hlens_bpe, h3, prev_c2b_alpha)
                ...
                if decode_choice == "bpe":
                    ...
                    subprev_alpha, hbpe_, char_output_all, charposterindex, char_predit_ey = self.char_onebpe_training(
                        ...
                        sampling_mask, char_predit_ey) # char_len[i,:] is batch
    ...
    def char_onebpe_training(self, exp_h, exp_hlens, prev_alpha, h_, yseq, char_len, char_output_all, charposterindex, ssmask, char_predit_ey, strm_idx=0):  # realmask
        ...
        for ii in range(int(max(char_len))):
            ...
            char_yprev = torch.where(ssmask, vy, char_predit_ey,) # [B,1]
            ey = self.dropout_emb_char(self.embed_char(char_yprev))
    ...
    def calculate_all_attentions(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, sampling_probability=0.0, strm_idx=0):  #### change here
        """Decoder forward
...
# based on decoders_base_mltaskrll_modif3, I add SS/decoder-dropout
# decoder_base_mltaskller_modification3_for --> decoder_base_newmltaskr_m3ss_for(mltaskrllm3ss)
def decoder_base_newmltaskr_m3ss_for(
