...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('This is multi-encoder multi-level mltaskr version')
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        SWITCH_CHAR = True
        self.batchcount += 1        
        [hs_pad_bpe, hs_pad_char] = hs_pad
        [hlensbpe, hlenschar] = hlens
        nbatch = hs_pad_bpe.shape[0]
        ...
        if decode_choice == "bpe":
            ...
            # pys: utt x olen (char)
            ys_char_in_pad  = pad_list(ys_char_in, self.eos_char).to(hs_pad_char).long() # pad feature torch.Tensor([B,decT])
            ys_char_out_pad = pad_list(ys_char_out, self.ignore_id).to(hs_pad_char).long() # pad output symbol
        ...
        if SWITCH_BPE == True:
            ...
            # padding for ys with -1
            # pys: utt x olen (bpe)
            ys_bpe_in_pad  = pad_list(ys_bpe_in, self.eos_bpe).to(hs_pad_bpe).long() # pad feature torch.Tensor([B,decT])
            ys_bpe_out_pad = pad_list(ys_bpe_out, self.ignore_id).to(hs_pad_bpe).long() # pad output symbol

            # pre-computation of embedding y -> emb(y) [B,C,D_senone]
            eys_bpe = self.dropout_emb_bpe(self.embed_bpe(ys_bpe_in_pad))  # utt x olen x zdim [B,decoder_T,300]

            # hlen should be list of integer
            hlens_bpe = list(map(int, hlensbpe))
            ...
            char_output_all = (1.0/self.odim_char) * torch.ones((nbatch, olength_char, self.odim_char)).to(hs_pad_char)
            ...
            for i in six.moves.range(olength_bpe):
                ...
                if decode_choice == "bpe":
                    ...
                    subprev_alpha, hbpe_, char_output_all, charposterindex = self.char_onebpe_training(
                        bpe_enc, hlens_char, subprev_alpha, hbpe_, ysequence, char_len[i,:], 
    ...
    def calculate_all_attentions(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0):  #### change here
        ...
        if SWITCH_BPE == True:
            ...
            # padding for ys with -1
            # pys: utt x olen (bpe)
            ys_bpe_in_pad  = pad_list(ys_bpe_in, self.eos_bpe).to(hs_pad_char).long() # pad feature torch.Tensor([B,decT])
            ys_bpe_out_pad = pad_list(ys_bpe_out, self.ignore_id).to(hs_pad_char).long() # pad output symbol
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        #############################################################################
        exp_hlenschar = hlens_char.repeat(beam).view(beam, batch).transpose(0, 1).contiguous()        
        exp_hlenschar = exp_hlenschar.view(-1).tolist()
        exp_h_char = char_h.unsqueeze(1).repeat(1, beam, 1, 1).contiguous()
        exp_h_char = exp_h_char.view(n_bb, char_h.size()[1], char_h.size()[2]) # [n_bb, T, eprojs]
        #############################################################################

        if (decode_choice == 'char'):
            # extra initialization        
            h_ = self.mean_init_char_state(exp_h_char, list(map(int, exp_hlenschar))) #.repeat(n_bb, 1) # [n_bb, dunits]
            prev_postr_energy = (to_device(self, make_trueframe_mask(exp_hlenschar))).float() # previous postr attention score
            ...
            hbpe_ = self.mean_init_bpe_state(exp_h_char, list(map(int, exp_hlenschar))) #.repeat(n_bb, 1) # [n_bb, dunits]
        ...
        for i in six.moves.range(maxlen):
            if decode_choice == 'char':
                ...
                ## 2. Attention eqn.(23): c(t)=AttentionContext(s'(t), enc_o) || dunits
                prior_att_c, prior_att_w = self.att_char[att_char_idx](exp_h_char, exp_hlenschar, h1, prev_alpha)
                ...
                prior_att_c, prior_att_w = self.att_bpe[att_bpe_idx](exp_h, exp_hlens, h1, prev_alpha)
                ...
                ## 2. Attention : c(t)=AttentionContext(s'(t), enc_o) 
                # || dunits (prior_att_c[B,eprojs] prior_att_w[B,T])
                c2b_prior_att_c, c2b_prior_att_w = self.att_char2bpe[att_bpe_idx](exp_h, exp_hlens, h3, prev_c2b_alpha)
            ...
            if (decode_choice == 'bpe'):
                local_scores_char = att_weight * self.char_onestep_decoder(exp_h_char, exp_hlenschar, subprev_alpha, hbpe_, yseq_char)
            ...
            if (decode_choice == 'bpe'): 
                ...
                local_scores_char, subprev_alpha, hbpe_ = self.char_onebpe_decoder(exp_h_char, exp_hlenschar, subprev_alpha, hbpe_, ysequence, char_len) # yseq_char
...
def decoder_multiencoder_mltaskller_for(
