...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('[decoder_base_mltaskller_modification3_for] This is info transformation from char to BPE (mltaskbaserllm3).')
        print('aux7r1v0 : 7 aux file + entropy -------------76:')
        print('-----------------------------------------------------------------------------------------------------------------')
        ...
        self.c2b_ctxcomb = torch.nn.Linear(cprojs, cprojs) 
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        if SWITCH_BPE == True:
            ...
            for i in six.moves.range(olength_bpe):
                ...
                INTER_CODE_PART = True
                if INTER_CODE_PART == True:
                    ## 1. RNN : s'(t)=RNN(y(t-1),s(t-1)) 
                    # || to combine [emb(t-1),state(t-1)] -> predt-state(t) 
                    # || RNN(dunits) : concat[ey,ctx-previous-posterior]
                    ...
                    ctx = torch.sigmoid( self.c2b_ctxcomb(prior_att_c) ) * c2b_prior_att_c
                    ctx_comb = torch.cat([prior_att_c, ctx], dim=1)

                    preact4 = self.char2bpe_rnn2_U_n1(h1h3_comb) # [B,dunits*2]=[10,600]
                    preact4 += self.char2bpe_rnn2_Wc(ctx_comb) # no bias [B,dunits*2]=[10,600]
                    ...
                    preactx4 *= r4
                    preactx4 += self.char2bpe_rnn2_Wcx(ctx_comb) # [B,dunits]=[10,300]
                ...
                if decode_choice == "bpe":
                    ...
                    subprev_alpha, hbpe_, char_output_all, charposterindex = self.char_onebpe_training(
                        bpe_enc, hlens_bpe, subprev_alpha, hbpe_, ysequence, char_len[i,:], 
                        char_output_all, charposterindex) # char_len[i,:] is batch
    ...
    def char_onebpe_training(self, exp_h, exp_hlens, prev_alpha, h_, yseq, char_len, char_output_all, charposterindex, strm_idx=0):  # realmask
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        for i in six.moves.range(maxlen):
            ...
            inter_h_ = torch.index_select(h4.view(n_bb, *h4.shape[1:]), 0, vidx) # 挑选所要的中间结果(3): state h2(30,800)
    ...
    #     CHOICE_SCORE_TYPES = recog_args.CHOICE_SCORE_TYPES
    #     if (recog_args.CHOICE_SCORE_TYPES == "bpe1char") or (recog_args.CHOICE_SCORE_TYPES == "bpe2char"):
    #         BPE_JointDecoding = True
    #         normalize_score = True
    #     elif (recog_args.CHOICE_SCORE_TYPES == "bpeonly"):
    #         BPE_JointDecoding = False
    #         normalize_score = False
    #     else:
    ...
    #     for i in six.moves.range(maxlen):
    #         if decode_choice == 'char':
    #             logging.debug('Split[mltask] CHAR position ' + str(i))
    #             vy = to_device(self, torch.LongTensor(get_last_yseq(yseq))) # get the last char
    #             ey = self.dropout_emb_char(self.embed_char(vy)) # [n_bb, embsize]
    #             xx_ = self.char_rnn1_Wx(ey) # state_belowx[n_bb, dunits]
    #             x_ = self.char_rnn1_W(ey) # [n_bb, 2*dunits]
    ...
    #             # || to combine [emb(t-1),state(t-1)] -> predt-state(t) 
    #             # || RNN(dunits) : concat[ey,ctx-previous-posterior]
    #             preact3 = self.char2bpe_rnn1_U(subh_) # no bias -> [B,dunits*2]=[10,600]
    #             preact3 += c2b_x_ # [B,dunits*2]=[10,600]
    #             preact3 = torch.sigmoid(preact3)
    #             r3 = self._slice(preact3, 0, self.dunits) # reset gate [B,dunits]=[10,300]
    #             u3 = self._slice(preact3, 1, self.dunits) # update gate [B,dunits]=[10,300]
    #             preactx3 = self.char2bpe_rnn1_Ux(subh_) # no bias [B,dunits]=[10,300]
    #             preactx3 *= r3
    #             preactx3 += c2b_xx_ 
    #             h3 = torch.tanh(preactx3)
    #             h3 = u3 * subh_ + (1. - u3) * h3
    ...
    #             ## 3. RNN : s(t)=RNN(c(t),s'(t)) 
    #             # || att_c(eprojs) z_infocomb_list(dunits)
    #             h3 = torch.sigmoid( self.c2b_statecomb(h3) ) + h1
    ...
    #         if (decode_choice == 'bpe') and (BPE_JointDecoding == True):
    #             local_scores_char = att_weight * self.char_onestep_decoder(exp_h, exp_hlens, subprev_alpha, subh_, yseq_char)
    #             local_scores_char = local_scores_char.view(batch, beam, self.odim_char)
    #             if i == 0: ############ ?????? ############
    #                 local_scores_char[:, 1:, :] = self.logzero
    ...
    #             subprev_alpha = torch.index_select(subprev_alpha.view(n_bb, *subprev_alpha.shape[1:]), 0, vidx)
    #             subh_ = torch.index_select(subh_.view(n_bb, *subh_.shape[1:]), 0, vidx) ############ 新增裁剪过程
    ...
    #             local_scores_char, subprev_alpha, subh_ = self.char_onebpe_decoder(exp_h, exp_hlens, subprev_alpha, subh_, ysequence, char_len) # yseq_char
    #             vscores += local_scores_char
    ...
    #     nbest_hyps = [sorted(ended_hyps[samp_i], key=lambda x: x['score'], reverse=True)[:min(len(ended_hyps[samp_i]), recog_args.nbest)] # recog_args.nbest (1)
    #                   for samp_i in six.moves.range(batch)]
    #     print("72:mltaskraux7 : decoder_base_mltaskller_modification3_aux7_for", BPE_JointDecoding)

    #     return nbest_hyps


# this commands function
def decoder_base_mltaskr_modification3_aux7r1v0_entropy_for(
