...
decode_choice = "bpe" # stable BPE

MAX_DECODER_OUTPUT = 5 # ?
CTC_SCORING_RATIO = 1.5 # ?
LONGEST_LENS = 10000
traditional_loss_calculate = False
BPE_JointDecoding = True
...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
            ...
            embsize_char=256, embsize_bpe=256):
        ...
        print('MLTASKR-NewBaseline :->: The decoder version is multi-level char&bpe (code checking -> second joint decoding).')
        print('Setting loss calc', traditional_loss_calculate)
        print('Setting BPE_JointDecoding', BPE_JointDecoding)
        print('This is just the comparable baseline [BPE & CHAR]. mltask.(decoders_new_mltaskr1.py-decoder_new_mltaskr1_for)')
        print('Deep Fusion :  v10rd... [h1;sigmoid(FC(h1))*h3].')
        print('[decoders_new_mltaskr1.py] This is just information transformation from CHAR to BPE (mtaskrn).')
        print('The additional RNN1 use the corresponding CHAR result. Index of yc(t-1) is same as h_.')
        ...
        self.classbpe_enc_W   = torch.nn.Linear(2*cprojs, dunits, bias=False)
        ...
        self.att_c2b = att_char2bpe
        ## CHAR2BPE RNN Layer 1
        self.c2b_rnn1_Wx = torch.nn.Linear(self.embsize_char, dunits)
        self.c2b_rnn1_W  = torch.nn.Linear(self.embsize_char, dunits*2)
        self.c2b_rnn1_U  = torch.nn.Linear(dunits, dunits*2, bias=False)
        self.c2b_rnn1_Ux = torch.nn.Linear(dunits, dunits, bias=False)
        ## CHAR2BPE RNN Layer 2
        self.c2b_rnn2_U_n1  = torch.nn.Linear(2*dunits, dunits*2)
        self.c2b_rnn2_Wc    = torch.nn.Linear(cprojs, dunits*2, bias=False)
        self.c2b_rnn2_Ux_n1 = torch.nn.Linear(2*dunits, dunits)
        self.c2b_rnn2_Wcx   = torch.nn.Linear(cprojs, dunits, bias=False)    
        ## Deep Fusion Like FC 
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        if PREPARE_USING_B2C == True:
            self.att_c2b[att_bpe_idx].reset()
        ...
        if SWITCH_CHAR == True: ## CHAR 序列一次性全部做完
            ...
            for i in six.moves.range(olength_char):
                ...
                charlmhist_storing.append(char_eyi)
                ...
                char_logit_all_for_classif = self.classchar_state_W(char_h2) + self.classchar_yprev_W(char_eyi) + self.classchar_enc_W(char_prior_att_c) # [B,dunits]=[10,300]
                char_logit_linear = torch.reshape( char_logit_all_for_classif, (batch, self.dunits//2, 2) )
                char_logit_clf, _ = torch.max( char_logit_linear, dim=-1 ) # [B,dunits//2]=[B,150]
                char_probs = self.char_class_W(char_logit_clf) # [B,odim]=[10,496]
                char_probs = F.softmax(char_probs, dim=-1) # [B,496]
            ...
            charlmhist_store = torch.stack(charlmhist_storing, dim=1) # .view(batch, olength_char, self.embchar)


        if SWITCH_BPE == True: ## 在这一组中采用index 来挑选所需要使用的(t-1) char state 
            ## The choosing y(t-1) embedding embchar(0) is sos
            char2bpe_yhist_emb = charlmhist_store[:,0,:] # emb char --> sos
            ...
            state_char2bpe_ = self.c2b_rnn1_W(state_bpe)  # [B,decT,dunits]=[10,144,600] 
            state_char2bpex = self.c2b_rnn1_Wx(state_bpe) # [B,decT,dunits]=[10,144,300]
            for i in six.moves.range(olength_bpe):
                ...
                if PREPARE_USING_B2C == True:
                    ## (1) RNN : s'(t)=RNN(y(t-1),s(t-1)) 
                    c2b_x_  = self.c2b_rnn1_W(char2bpe_yhist_emb) # state_char2bpe_[:,i,:] 
                    c2b_xx_ = self.c2b_rnn1_Wx(char2bpe_yhist_emb) # state_char2bpex[:,i,:]      

                    c2b_preact3 = self.c2b_rnn1_U(char2bpe_state_h_) # no bias -> [B,dunits*2]=[10,600]
                    ...
                    c2b_preactx3 = self.c2b_rnn1_Ux(char2bpe_state_h_) # no bias [B,dunits]=[10,300]
                    c2b_preactx3 *= c2b_r3
                    ...
                    ## (2) Attention : c(t)=AttentionContext(s'(t), enc_o) 
                    c2b_att_c, c2b_att_w = self.att_c2b[att_bpe_idx](bpe_enc, hlens_bpe, c2b_h3, prev_c2b_alpha)
                    c2b_att_weights.append(c2b_att_w)
                    c2b_att_context.append(c2b_att_c)
                    ## (3) RNN : s(t)=RNN(c(t),s'(t)) 
                    h1h3_comb = torch.sigmoid( self.c2b_statecomb(bpe_h1) ) * c2b_h3
                    h1h3_comb = torch.cat([bpe_h1, h1h3_comb], dim=1)
                    c2b_preact4 = self.c2b_rnn2_U_n1(h1h3_comb) # [B,dunits*2]=[10,600]
                    c2b_preact4 += self.c2b_rnn2_Wc(c2b_att_c) # no bias [B,dunits*2]=[10,600]
                    ...
                    c2b_preactx4 = self.c2b_rnn2_Ux_n1(h1h3_comb) # [B,dunits]=[10,300]
                    c2b_preactx4 *= c2b_r4
                    c2b_preactx4 += self.c2b_rnn2_Wcx(c2b_att_c) # [B,dunits]=[10,300]
                    ...
                    context = torch.cat([bpe_prior_att_c, c2b_att_c],dim=1)
                    cstates = torch.cat([bpe_h2, c2b_h4],dim=1)
                    ############################## this is interaction operation         
                ## 4. choice the right index char-state, csum-1 -> clamp(0,...)
                cindex = torch.clamp(csum_charlen[i,:], min=0, max=olength_char-1).unsqueeze(1)
                if not (i == (olength_bpe - 1)):
                    char2bpe_state_h_  = self.batched_index_select(input=charstate_store, dim=1, index=cindex).squeeze(1)
                    char2bpe_yhist_emb = self.batched_index_select(input=charlmhist_store, dim=1, index=cindex).squeeze(1)
                ## 5. Classification for every frames
                # h2[B,dunits]  state_bpe[B,decT]  prior_att_c[B,dunits//2]
                bpe_logit_all_for_classif = self.classbpe_state_W(cstates) + self.classbpe_yprev_W(bpe_eyi) + self.classbpe_enc_W(context) # [B,dunits]=[10,300]
                bpe_logit_linear = torch.reshape( bpe_logit_all_for_classif, (batch, self.dunits//2, 2) )
                bpe_logit_clf, _ = torch.max( bpe_logit_linear, dim=-1 ) # [B,dunits//2]=[B,150]
                bpe_probs = self.bpe_class_W(bpe_logit_clf) # [B,odim]=[10,496]
                bpe_probs = F.softmax(bpe_probs, dim=-1) # [B,496]
                ## 6. save posterior output
                bpe_output_all.append(bpe_probs)
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        if (decode_choice == 'bpe') and (BPE_JointDecoding == True):
        ...
        if decode_choice == 'bpe':
            ...
            if (BPE_JointDecoding == True): self.att_char[att_char_idx].reset()
        ...
        if (decode_choice == 'char'):
            ...
            if (BPE_JointDecoding == True): # for char sequence
        ...
        for i in six.moves.range(maxlen):
            if decode_choice == 'char':
                ...
                c2b_xx_ = self.c2b_rnn1_Wx(ey) # state_belowx[n_bb, dunits]
                c2b_x_  = self.c2b_rnn1_W(ey)                
                cc_ = exp_h # [n_bb, T, eprojs]
                ...
                ## 1. RNN : s'(t)=RNN(y(t-1),s(t-1)) 
                # yseq_char_inbpe is list
                hist_ychar = torch.tensor(yseq_char_inbpe, dtype=torch.long).cuda()
                charey = self.dropout_emb_bpe(self.embed_bpe(hist_ychar))

                preact3 = self.c2b_rnn1_U(hbpe_) # no bias -> [B,dunits*2]=[10,600]
                preact3 += self.c2b_rnn1_W(charey) # c2b_x_ # [B,dunits*2]=[10,600]
                ...
                preactx3 = self.c2b_rnn1_Ux(hbpe_) # no bias [B,dunits]=[10,300]
                preactx3 *= r3
                preactx3 += self.c2b_rnn1_Wx(charey) # c2b_xx_ 
                h3 = torch.tanh(preactx3)
                h3 = u3 * hbpe_ + (1. - u3) * h3
                ## 2. Attention
                c2b_prior_att_c, c2b_prior_att_w = self.att_c2b[att_bpe_idx](cc_, exp_hlens, h3, prev_c2b_alpha)
                ## 3. RNN : s(t)=RNN(c(t),s'(t)) 
                h1h3_comb = torch.sigmoid( self.c2b_statecomb(h1) ) * h3
                h1h3_comb = torch.cat([h1, h1h3_comb], dim=1)
                preact4 = self.c2b_rnn2_U_n1(h1h3_comb) # [B,dunits*2]=[10,600]
                preact4 += self.c2b_rnn2_Wc(c2b_prior_att_c) # no bias [B,dunits*2]=[10,600]
                preact4 = torch.sigmoid(preact4)
                r4 = self._slice(preact4, 0, self.dunits) # [B,dunits]
                u4 = self._slice(preact4, 1, self.dunits) # [B,dunits]
                preactx4 = self.c2b_rnn2_Ux_n1(h1h3_comb) # [B,dunits]=[10,300]
                preactx4 *= r4
                preactx4 += self.c2b_rnn2_Wcx(c2b_prior_att_c) # [B,dunits]=[10,300]
            ...
            if (decode_choice == 'bpe') and (BPE_JointDecoding == True):
                ...
                # eos_vscores_char = local_scores_char[:, :, self.eos_char] + vscores_char
                # eos_vscores      = local_scores[:, :, eos_token]          + vscores ######+ vscores_char
                y_prev_char      = yseq_char[:][:]
            else:                
                eos_vscores      = local_scores[:, :, eos_token] + vscores # default setting (no extra char)
            ######################################################################################
            ...
            if (decode_choice == 'bpe') and (BPE_JointDecoding == True): 
                ...
                ### 准备每一个y的-1的情况：还要防止“空”的情况--> 就照抄前面一个
                ...
                # vscores_char += local_scores_char
