...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('Deep Fusion :  v10rd... concat-[h1;sigmoid(FC(h1))*h3].')
        print('The embsize_bpe is %d while embsize_char is %d.' %(embsize_bpe, embsize_char))
        print('[decoders_base_mltaskrll-modification3-concat.py] This is just information transformation from char to BPE (mltaskbaserll-modification3-concat).')
        print('这个版本还是保持原先的单向属性  modification3 + concat-GLU. 实际上为了求稳把Concat删除了')
        print('这个版本是：让BPE来产生种子，之后相对应的调整char的----char不会完全随机')
        print('------------------------------------------------------------------------------------------------------------------------------------------------')
    ...
    # Forward Process
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, sampling_probability, strm_idx=0): 
        """Decoder forward
        ...
        if decode_choice == "bpe":
            ...
            # teacherforcing_charmask = torch.zeros((nbatch, ys_char_out_pad.size(1))).to(hs_pad)
        ...
        if SWITCH_BPE == True:
            ...
            teacherforcing_bpemask = torch.zeros((nbatch, ys_bpe_out_pad.size(1))).to(hs_pad)
            # sampling_probability = 0.5 ########## masked
            # create schedule sampling mask - : 1. the bpe probs 2. the corresponding char
            for i in six.moves.range(ys_bpe_out_pad.size(1)): # 1 表示是Predict-output
                if i == 0:
                    teacherforcing_bpemask[:, i] = 0
                else:
                    mask_onestep = [random.random() for _ in range(nbatch)] # (nbatch)
                    mask = [int(probs<sampling_probability) for probs in mask_onestep]
                    teacherforcing_bpemask[:, i] = torch.tensor( mask ).to(teacherforcing_bpemask)
                i = i+1
            
            if ( (self.batchcount%2000)==0 ): print('Using sampling_probability with %2.1f' %(sampling_probability) )
            ...
            ## for char schedule sampling output saving
            char_predit_ey = self.eos_char * torch.ones((nbatch)).to(hs_pad).long()

            for i in six.moves.range(olength_bpe):
                ## schedule sampling mask
                schedulesampling_mask_ = teacherforcing_bpemask[:,i] # 为0表示采用的是真实 gold label / 为1表示模型输出
                m_  = y_bpe_mask[:,i] # the variables for this bpe decoder steps
                cc_ = bpe_enc # [B,decT,dunit//2]
                ## 0. Use teacherforcing_bpemask to choice which y_hist is used.
                if i > 0:
                    z_out = np.argmax(probs.detach().cpu(), axis=1)
                    bpe_prev_ey_pred = self.dropout_emb_bpe(self.embed_bpe(to_device(self, z_out))) # (batch, embsize)
                    bpe_prev_ey = schedulesampling_mask_.unsqueeze(1)*bpe_prev_ey_pred                   + (1.-schedulesampling_mask_).unsqueeze(1)*state_bpe[:,i,:]
                    x_          = schedulesampling_mask_.unsqueeze(1)*self.bpe_rnn1_W(bpe_prev_ey)       + (1.-schedulesampling_mask_).unsqueeze(1)*state_bpe_[:,i,:]
                    xx_         = schedulesampling_mask_.unsqueeze(1)*self.bpe_rnn1_Wx(bpe_prev_ey)      + (1.-schedulesampling_mask_).unsqueeze(1)*state_bpex[:,i,:]
                    c2b_x_      = schedulesampling_mask_.unsqueeze(1)*self.char2bpe_rnn1_W(bpe_prev_ey)  + (1.-schedulesampling_mask_).unsqueeze(1)*state_char2bpe_[:,i,:]
                    c2b_xx_     = schedulesampling_mask_.unsqueeze(1)*self.char2bpe_rnn1_Wx(bpe_prev_ey) + (1.-schedulesampling_mask_).unsqueeze(1)*state_char2bpex[:,i,:]
                else:
                    bpe_prev_ey = state_bpe[:, i, :]
                    x_  = state_bpe_[:,i,:]
                    xx_ = state_bpex[:,i,:]
                ...
                logit_all_for_classif = self.classbpe_state_W(cstates) + self.classbpe_yprev_W(bpe_prev_ey) + self.classbpe_enc_W(context) # [B,dunits]=[10,300]
                ...
                if decode_choice == "bpe":
                    ...
                    subprev_alpha, hbpe_, char_output_all, charposterindex, char_predit_ey = self.char_onebpe_training(
                        bpe_enc, hlens_bpe, subprev_alpha, hbpe_, ysequence, char_len[i,:], char_output_all, charposterindex, 
                        schedulesampling_mask_, char_predit_ey) # char_len[i,:] is batch
    ...
    def char_onebpe_training(self, exp_h, exp_hlens, prev_alpha, h_, yseq, char_len, char_output_all, charposterindex, ssmask, char_predit_ey, strm_idx=0):  # realmask
        ...
        for ii in range(int(max(char_len))):
            ## -. schedule sampling mask 
            pred_ey = self.dropout_emb_char(self.embed_char(to_device(self, char_predit_ey))) # [nbatch, embsize]
            vy      = yseq[:,ii] # to_device(self, torch.LongTensor((yseq[:,i])))         # get the last char
            gold_ey = self.dropout_emb_char(self.embed_char(vy)) # [nbatch, embsize]
            ey      = ssmask.unsqueeze(1)*pred_ey + (1. - ssmask).unsqueeze(1)*gold_ey # 1表示采用预测的 / 0表示的是真实的 
            ...
            ## 6. update the last useful output
            tmp_vy = to_device( self, np.argmax(probs.detach().cpu(), axis=1) ).long() # [B] cuda--
            char_predit_ey = m_.long() * tmp_vy + (1. - m_).long() * char_predit_ey # 如果m_是1的话就拿来更新，反之则保留原先的数值 [B]
        ...
        return prev_alpha, h_, char_output_all, charposterindex, char_predit_ey



    def calculate_all_attentions(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, sampling_probability, strm_idx=0):  #### change here
        """Decoder forward
        ...
        if SWITCH_BPE == True:
            ...
            ys_bpe_out_pad = pad_list(ys_bpe_out, self.ignore_id).to(hs_pad).long() # pad output symbol
            teacherforcing_bpemask = torch.zeros((nbatch, ys_bpe_out_pad.size(1))).to(hs_pad)
            sampling_probability = 0.0 ########## masked
            # create schedule sampling mask - : 1. the bpe probs 2. the corresponding char
            for i in six.moves.range(ys_bpe_out_pad.size(1)): # 1 表示是Predict-output
                if i == 0:
                    teacherforcing_bpemask[:, i] = 0
            ...
            state_char2bpex = self.char2bpe_rnn1_Wx(eys_bpe) # [B,decT,dunits]=[10,144,300]
            ## for char schedule sampling output saving
            char_predit_ey = self.eos_char * torch.zeros((nbatch)).to(hs_pad).long()
            ...
            for i in six.moves.range(olength_bpe):
                ...
                if i > 0:
                    z_out = np.argmax(probs.detach().cpu(), axis=1)
                    bpe_prev_ey = self.dropout_emb_bpe(self.embed_bpe(to_device(self, z_out)))
                    x_      = schedulesampling_mask_.unsqueeze(1)*self.bpe_rnn1_W(bpe_prev_ey)       + (1.-schedulesampling_mask_).unsqueeze(1)*state_bpe_[:,i,:]
                    xx_     = schedulesampling_mask_.unsqueeze(1)*self.bpe_rnn1_Wx(bpe_prev_ey)      + (1.-schedulesampling_mask_).unsqueeze(1)*state_bpex[:,i,:]
                    c2b_x_  = schedulesampling_mask_.unsqueeze(1)*self.char2bpe_rnn1_W(bpe_prev_ey)  + (1.-schedulesampling_mask_).unsqueeze(1)*state_char2bpe_[:,i,:]
                    c2b_xx_ = schedulesampling_mask_.unsqueeze(1)*self.char2bpe_rnn1_Wx(bpe_prev_ey) + (1.-schedulesampling_mask_).unsqueeze(1)*state_char2bpex[:,i,:]
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        for i in six.moves.range(maxlen):
            ...
            if CHOICE_SCORE_TYPES == "bpeonly":
                ...
                print("SS --- ERROR HERE FOR USING CHOICE_SCORE_TYPES %s" %(CHOICE_SCORE_TYPES))
                exit(0)
...
def decoder_base_mltaskller_m3c_schedulesampling_for(
