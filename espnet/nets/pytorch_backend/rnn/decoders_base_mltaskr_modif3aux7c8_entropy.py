...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('c8 (the r3 in WSJ is similar here.)')
        print('-----------------------------------------------------------------------------------------------------------------')
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        if SWITCH_BPE == True:
            ...
            for i in six.moves.range(olength_bpe):
                ...
                # || to combine [emb(t-1),state(t-1)] -> predt-state(t) 
                # || RNN(dunits) : concat[ey,ctx-previous-posterior]
                info = torch.sigmoid( self.c2b_statecomb2(inter_h_) )*hbpe_ + hbpe_
        ...
        return [self.loss_char, self.loss_bpe, self.loss_aux+self.loss_aux2], [acc_char, acc_bpe, acc_aux], [(bpe_entropy.sum(1).mean(0)), (c2b_entropy.sum(1).mean(0))]
...
def decoder_base_mltaskr_modification3_aux7c8_entropy_for(
