...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('aux7 : 7 aux file + entropy')
        print('c9 : Real Double GLU')
        self.c2b1_statecomb_FC1 = torch.nn.Linear(dunits, dunits)
        self.c2b1_statecomb_FC2 = torch.nn.Linear(dunits, dunits)
        self.c2b2_statecomb_FC1 = torch.nn.Linear(dunits, dunits)
        self.c2b2_statecomb_FC2 = torch.nn.Linear(dunits, dunits)
        print('-----------------------------------------------------------------------------------------------------------------')
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        if SWITCH_BPE == True:
            ...
            for i in six.moves.range(olength_bpe):
                ...
                # || to combine [emb(t-1),state(t-1)] -> predt-state(t) 
                # || RNN(dunits) : concat[ey,ctx-previous-posterior]
                hbpe_ = torch.sigmoid( self.c2b1_statecomb_FC1(inter_h_) ) * self.c2b1_statecomb_FC2(hbpe_)
                ...
                ## 3. RNN : s(t)=RNN(c(t),s'(t)) 
                # || att_c(eprojs) z_infocomb_list(dunits)
                h3 = torch.sigmoid( self.c2b2_statecomb_FC1(h1) ) * self.c2b2_statecomb_FC2(h3)
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        nbest_hyps = [sorted(ended_hyps[samp_i], key=lambda x: x['score'], reverse=True)[:min(len(ended_hyps[samp_i]), recog_args.nbest)] # recog_args.nbest (1)
                      for samp_i in six.moves.range(batch)]
        print('[mltaskraux7c9] The value of CHOICE_SCORE_TYPES is %s. TJ-USING.' %(CHOICE_SCORE_TYPES))
...
def decoder_base_mltaskr_modification3_aux7c9_entropy_for(
