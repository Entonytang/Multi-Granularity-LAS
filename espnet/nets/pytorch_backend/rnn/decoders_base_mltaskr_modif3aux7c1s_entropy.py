...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('aux7c1s : 7 aux file + entropy -----> 7auxc1s')
        print('-----------------------------------------------------------------------------------------------------------------')
        ...
        self.char2bpe_rnn1_U1  = torch.nn.Linear(dunits, dunits*2, bias=False)
        self.char2bpe_rnn1_Ux1 = torch.nn.Linear(dunits, dunits, bias=False)        
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        if SWITCH_BPE == True:
            ...
            for i in six.moves.range(olength_bpe):
                ...
                preact3 += self.char2bpe_rnn1_U1(inter_h_)
                ...
                preactx3 *= r3
                preactx3 += c2b_xx_ 
                preactx3 += self.char2bpe_rnn1_Ux1(inter_h_)
                h3 = torch.tanh(preactx3)
                h3 = u3 * hbpe_ + (1. - u3) * h3
                h3 = m_.unsqueeze(1) * h3 + (1. - m_).unsqueeze(1) * hbpe_

                ## 2. Attention : c(t)=Attention-Context(s'(t), enc_o) 
                # || dunits (prior_att_c[B,eprojs] prior_att_w[B,T])
                ...
                h1_comb_h3 = torch.cat([h1, h3, hc],dim=1)
                preact4 = self.char2bpe_rnn2_U_n1(h1_comb_h3) # [B,dunits*2]=[10,600]
                ...
                preactx4 = self.char2bpe_rnn2_Ux_n1(h1_comb_h3) # [B,dunits]=[10,300]
                preactx4 *= r4
...
def decoder_base_mltaskr_modification3_aux7c1_entropy_for(
