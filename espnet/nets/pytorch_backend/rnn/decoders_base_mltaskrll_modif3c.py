...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('这个版本还是保持原先的单向属性  modification3 + concat-GLU.')
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        if SWITCH_BPE == True:
            ...
            for i in six.moves.range(olength_bpe):
                ...
                ## 3. RNN : s(t)=RNN(c(t),s'(t)) 
                # || att_c(eprojs) z_infocomb_list(dunits)
                h3 = torch.sigmoid( self.c2b_statecomb(h3) )
                h3c = torch.cat([h1, h3], dim=1)
                preact4 = self.char2bpe_rnn2_U_n1(h3c) # [B,dunits*2]=[10,600]
                ...
                preactx4 = self.char2bpe_rnn2_Ux_n1(h3c) # [B,dunits]=[10,300]
                preactx4 *= r4
...
def decoder_base_mltaskller_modification3c_for(
