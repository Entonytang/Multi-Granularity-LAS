...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
                    ...
                    embsize_char=256, embsize_bpe=1000): ## RIGHT(28-28)
        ...
        ## Classification Part ##
        self.charclass_dunits = 1024
        self.classchar_state_W = torch.nn.Linear(dunits, self.charclass_dunits*2)
        self.classchar_yprev_W = torch.nn.Linear(self.embsize_char, self.charclass_dunits*2, bias=False)
        self.classchar_enc_W   = torch.nn.Linear(eprojs, self.charclass_dunits*2, bias=False)
        self.char_class_W    = torch.nn.Linear(self.charclass_dunits, odim_char)
        ## Constant value
        ...
        print('[decoders_base_mltaskrll-modification1.py] This is just information transformation from char to BPE (mltaskbaserll-modification1).')
        print('这个版本还是保持原先的单向属性吧：')
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        nbest_hyps = [sorted(ended_hyps[samp_i], key=lambda x: x['score'], reverse=True)[:min(len(ended_hyps[samp_i]), recog_args.nbest)] # recog_args.nbest (1)
                      for samp_i in six.moves.range(batch)]
        print('[mltaskrll-modification-1] The value of CHOICE_SCORE_TYPES is %s. TJ-USING.' %(CHOICE_SCORE_TYPES))
...
def decoder_base_mltaskller_modification1_for(
