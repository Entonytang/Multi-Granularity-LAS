...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('This is just the comparable baseline [BPE & CHAR]. mltask.(decoders_new_mltaskr0.py-decoder_new_mltaskr0_for)')
        print('Deep Fusion :  v10rd... [h1;sigmoid(FC(h1))*h3].')
        print('[decoders_new_mltaskr0.py] This is just information transformation from CHAR to BPE (mtaskrn).')
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        if recog_args.decode_choice == 'char': 
            odim_nums = self.odim_char 
        elif recog_args.decode_choice == 'bpe': 
        ...
        if (recog_args.decode_choice == 'bpe') and (BPE_JointDecoding == True):
        ...
        if recog_args.decode_choice == 'bpe':
            self.att_bpe[att_bpe_idx].reset()
            self.att_c2b[att_bpe_idx].reset()
