...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('[decoders_base_mltaskrllm3_ss.py] This is just information transformation from char to BPE (mltaskbaserll-modification3).')
        print('                                     + espresso Scheduled Sampling.')
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, sampling_probability=0.0, strm_idx=0): 
        ...
        if SWITCH_BPE == True:
            ...
            for i in six.moves.range(olength_bpe):
                ...
                if i > 0 and random.random() < sampling_probability: ## this is cpu SS version
                    logging.info(" scheduled sampling ") ##
                    z_out = np.argmax(probs.detach().cpu(), axis=1)
                    bpe_prev_ey = self.dropout_emb_bpe(self.embed_bpe(to_device(self, z_out)))
                    x_      = self.bpe_rnn1_W(bpe_prev_ey)
                    xx_     = self.bpe_rnn1_Wx(bpe_prev_ey)
                    c2b_x_  = self.char2bpe_rnn1_W(bpe_prev_ey)
                    c2b_xx_ = self.char2bpe_rnn1_Wx(bpe_prev_ey)           
                else:
                ...
                if decode_choice == "bpe":
                    ...
                    subprev_alpha, hbpe_, char_output_all, charposterindex = self.char_onebpe_training(
                        ...
                        char_output_all, charposterindex, sampling_probability) # char_len[i,:] is batch
    ...
    def char_onebpe_training(self, exp_h, exp_hlens, prev_alpha, h_, yseq, char_len, char_output_all, charposterindex, sampling_probability, strm_idx=0):  # realmask
        ...
        for ii in range(int(max(char_len))):
            ...
            if ii > 0 and random.random() < sampling_probability: ## this is cpu SS version
                logging.info(" scheduled sampling ") ##
                vy = np.argmax(probs.detach().cpu(), axis=1)
                ey = self.dropout_emb_char(self.embed_char(to_device(self, vy)))
            else:
...
def decoder_base_mltaskller_modification3_for(
