...
CHOICE_SCORE_TYPES = "bpeonly" # "bpe1char" / "bpe2char" "bpeonly"
decode_choice = "bpe"
...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
            ...
            embsize_char=128, embsize_bpe=1000
            ): ## RIGHT(28-28)
        ...
        print('-----------------------------------------------------------------------------------------------------------------')
        print('The decoder version is multi-level char&bpe (joint decoding).')
        print('The lsm_weight value is %f' %(lsm_weight))
        print('Deep Fusion :  v10rd... [h1;sigmoid(FC(h1))*h3].')
        print('Setting the decoder dropout is %2.1f' %(dropout))
        print('The embsize_bpe is %d while embsize_char is %d.' %(embsize_bpe, embsize_char))
        print('[decoder_base_mltaskller_modification3_for] This is info transformation from char to BPE (mltaskr_modi3a7c1v2n).')
        print('mltaskr_modi3a7c1v4n : 7 aux file + entropy (new)')
        print('-----------------------------------------------------------------------------------------------------------------')
        ...
        ###### char-to-bpe parameter for interaction (start)  ###### 
        ...
        self.char2bpe_rnn2_U_n1  = torch.nn.Linear(3*dunits, dunits*2)
        self.char2bpe_rnn2_Wc    = torch.nn.Linear(cprojs, dunits*2, bias=False)
        self.char2bpe_rnn2_Ux_n1 = torch.nn.Linear(3*dunits, dunits)
        self.char2bpe_rnn2_Wcx   = torch.nn.Linear(cprojs, dunits, bias=False)    
        ## Deep Fusion Like FC 
        self.c2b_statecomb = torch.nn.Linear(dunits, dunits)    
        ###### char-to-bpe parameter for interaction (finish) ###### 

        ## New added here by jtang ##
        self.init_bpe_state_fc = torch.nn.Linear(cprojs, dunits)
        ## BPE classification later
        self.classbpe_state_W = torch.nn.Linear(2*dunits, dunits)
        self.classbpe_yprev_W = torch.nn.Linear(self.embsize_bpe, dunits, bias=False)
        self.classbpe_enc_W   = torch.nn.Linear(cprojs, dunits, bias=False)
        self.bpe_class_W = torch.nn.Linear(dunits//2, odim_bpe)
        ## Constant value
        ...
        #########################
        self.inter_class_state = torch.nn.Linear(dunits, odim_bpe, bias=False)
        self.inter_class_context = torch.nn.Linear(dunits, odim_bpe)
        #########################        

        #########################
        self.bpe_class_state = torch.nn.Linear(dunits, odim_bpe, bias=False)
        self.bpe_class_context = torch.nn.Linear(dunits, odim_bpe)
        #########################        
    ...
    # egs. input[B,T,D] dim[1] index[B,1]
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        SWITCH_BPE = True
        SWITCH_CHAR = True
        PREPARE_SWITCH_BPE = True
        PREPARE_SWITCH_CHAR = True  
        PREPARE_USING_B2C = True
        inter_output_all = []       
        aux2_att_state = []
        aux_att_state = []

        if PREPARE_SWITCH_CHAR == True:
            ###### Codes for CHAR decoder and the additional operation (start)  ######
           ...
           # char y-label length
            ...
            char_output_all = []            
            # Unchanged varibles
            char_enc = hs_pad.clone()
            char_h_ = self.mean_init_char_state(char_enc, hlens_char) # hlens_char ?


        if PREPARE_SWITCH_BPE == True:
           ...
           # bpe y-label length
            ...
            bpe_output_all = []   
            # Unchanged varibles
            bpe_enc = hs_pad.clone()
            bpe_h_ = self.mean_init_bpe_state(bpe_enc, hlens_bpe) # hlens_bpe ?            


        if PREPARE_USING_B2C == True:
            ...
            char_len = np.zeros((decTlen, batch), dtype=np.int)

            yseq_char_inbpe = [[] for _ in range(decTlen)]
            yseq_char_inbpe[0] = [[self.sos_char] for y in six.moves.range(batch)] ## 只是为了循环体的变量
            ...
            charposterindex = to_device(self, -1*torch.ones( (batch,1) )).long()
            # cumsum lens  h2(1) == h_(2)
            # char_h_ init doesn't use csum_charlen to get. (just use this index to update it.)
            # eg [1,3,4,2,1] -> csum[1,4,8,10,11] [0:csum(0)->h_(1) 1:csum(1)->h_(4)]
            # eg [0,1,2,4,1] -> csum[0,1,3,7,8] [0:csum(0)->h_(0) 1:csum(1)->h_(1) 2:csum(2)->h_(3)]
            csum_charlen = torch.from_numpy(np.cumsum(char_len, axis=0)).cuda()
            ## used char2bpe hidden state vector
            char2bpe_state_h_ = char_h_
            c2b_prev_energy = (to_device(self, make_trueframe_mask(hlens_char))).float() # previous postr attention score
            prev_c2b_alpha = c2b_prev_energy / (c2b_prev_energy.sum(1, keepdim=True)+1e-8) # [B,T]   
            c2b_att_weights = []
            c2b_att_context = []
            # saving the char
            charstate_storing = []            
        ...
        if SWITCH_CHAR == True: ## CHAR 序列一次性全部做完
            char_prev_postr_energy = (to_device(self, make_trueframe_mask(hlens_char))).float() # previous postr attention score
            char_prev_alpha = char_prev_postr_energy / (char_prev_postr_energy.sum(1, keepdim=True)+1e-8) # [B,T]            
            # This is for the char decoder process
            # pre-computation of embedding y -> emb(y) [B,C,D_senone]
            state_char  = self.dropout_emb_char(self.embed_char(ys_char_in_pad))                   # y(t-1): [B,decT,embsize_char]=[B,144,256]
            state_char_ = self.char_rnn1_W(state_char)  # [B,decT,dunits]=[10,144,600]
            state_charx = self.char_rnn1_Wx(state_char) # [B,decT,dunits]=[10,144,300]                  
            for i in six.moves.range(olength_char):
                char_m_  = y_char_mask[:,i] # the variables for this char decoder steps
                char_eyi = state_char[:,i,:]
                char_x_  = state_char_[:,i,:] # self.char_rnn1_W(char_eyi) 
                char_xx_ = state_charx[:,i,:] # self.char_rnn1_Wx(char_eyi)                 

                ## 0.saving all char state sequence
                charstate_storing.append(char_h_)
                ## 1. RNN : s'(t)=RNN(y(t-1),s(t-1)) 
                # || to combine [emb(t-1),state(t-1)] -> predt-state(t) 
                # || RNN(dunits) : concat[ey,ctx-previous-posterior]
                char_preact1 = self.char_rnn1_U(char_h_) # no bias -> [B,dunits*2]=[10,600]
                char_preact1 += char_x_ # [B,dunits*2]=[10,600]
                char_preact1 = torch.sigmoid(char_preact1)
                char_r1 = self._slice(char_preact1, 0, self.dunits) # reset gate [B,dunits]=[10,300]
                char_u1 = self._slice(char_preact1, 1, self.dunits) # update gate [B,dunits]=[10,300]
                char_preactx1 = self.char_rnn1_Ux(char_h_) # no bias [B,dunits]=[10,300]
                char_preactx1 *= char_r1
                char_preactx1 += char_xx_ 
                char_h1 = torch.tanh(char_preactx1)
                char_h1 = char_u1 * char_h_ + (1. - char_u1) * char_h1
                char_h1 = char_m_.unsqueeze(1) * char_h1 + (1. - char_m_).unsqueeze(1) * char_h_
                ## 2. Attention : c(t)=AttentionContext(s'(t), enc_o) 
                # || dunits (prior_att_c[B,eprojs] prior_att_w[B,T])
                char_prior_att_c, char_prior_att_w = self.att_char[att_char_idx](char_enc, hlens_char, char_h1, char_prev_alpha)
                char_prior_att_weights.append(char_prior_att_w)
                char_prior_att_context.append(char_prior_att_c)
                ## 3. RNN : s(t)=RNN(c(t),s'(t)) 
                # || att_c(eprojs) z_infocomb_list(dunits)
                char_preact2 = self.char_rnn2_U_n1(char_h1) # [B,dunits*2]=[10,600]
                char_preact2 += self.char_rnn2_Wc(char_prior_att_c) # no bias [B,dunits*2]=[10,600]
                char_preact2 = torch.sigmoid(char_preact2)
                char_r2 = self._slice(char_preact2, 0, self.dunits) # [B,dunits]
                char_u2 = self._slice(char_preact2, 1, self.dunits) # [B,dunits]
                char_preactx2 = self.char_rnn2_Ux_n1(char_h1) # [B,dunits]=[10,300]
                char_preactx2 *= char_r2
                char_preactx2 += self.char_rnn2_Wcx(char_prior_att_c) # [B,dunits]=[10,300]
                char_h2 = torch.tanh(char_preactx2)
                char_h2 = char_u2 * char_h1 + (1. - char_u2) * char_h2
                char_h2 = char_m_.unsqueeze(1) * char_h2 + (1. - char_m_).unsqueeze(1) * char_h1 # [B,dunits]=[10,300]
                ## 4. Classification for every frames
                # h2[B,dunits]  state_char[B,decT]  prior_att_c[B,dunits//2]
                logit_char = self.classchar_state_W(char_h2) + self.classchar_enc_W(char_prior_att_c)
                logit_char = self.classchar_state_W(char_h2) + self.classchar_yprev_W(char_eyi) + self.classchar_enc_W(char_prior_att_c) # [nbatch,dunits]
                logit_char = torch.reshape( logit_char, (batch, self.dunits//2, 2) )
                logit_char, _ = torch.max( logit_char, dim=2 )   
                logit_char = self.char_class_W(logit_char)
                char_probs = F.softmax(logit_char, dim=-1) # [B,496]
                ## 5. save posterior output
                char_output_all.append(char_probs)
                ## 6. update previous variables
                char_h_ = char_h2
                char_prev_alpha = char_prior_att_w

            ## transform the information csum_charlen is (44,2)
            charstate_store = torch.stack(charstate_storing, dim=1) # .view(batch, olength_char, self.dunits)


        if SWITCH_BPE == True: ## 在这一组中采用index 来挑选所需要使用的(t-1) char state 
            bpe_prev_postr_energy = (to_device(self, make_trueframe_mask(hlens_bpe))).float() # previous postr attention score
            bpe_prev_alpha = bpe_prev_postr_energy / (bpe_prev_postr_energy.sum(1, keepdim=True)+1e-8) # [B,T]     
            # This is for the bpe decoder process
            # pre-computation of embedding y -> emb(y) [B,C,D_senone]
            state_bpe = self.dropout_emb_bpe(self.embed_bpe(ys_bpe_in_pad))  # utt x olen x zdim [B,decoder_T,300]            
            state_bpe_ = self.bpe_rnn1_W(state_bpe)  # [B,decT,dunits]=[10,144,600]
            state_bpex = self.bpe_rnn1_Wx(state_bpe) # [B,decT,dunits]=[10,144,300]            
            # about the addition rnn b2c operation
            state_char2bpe_ = self.char2bpe_rnn1_W(state_bpe)  # [B,decT,dunits]=[10,144,600] 
            state_char2bpex = self.char2bpe_rnn1_Wx(state_bpe) # [B,decT,dunits]=[10,144,300]
            for i in six.moves.range(olength_bpe):
                bpe_m_  = y_bpe_mask[:,i] # the variables for this bpe decoder steps
                bpe_x_  = state_bpe_[:,i,:]
                bpe_xx_ = state_bpex[:,i,:]
                bpe_eyi = state_bpe[:,i,:]
                ## 1. RNN : s'(t)=RNN(y(t-1),s(t-1)) 
                # || to combine [emb(t-1),state(t-1)] -> predt-state(t) 
                # || RNN(dunits) : concat[ey,ctx-previous-posterior]
                bpe_preact1 = self.bpe_rnn1_U(bpe_h_) # no bias -> [B,dunits*2]=[10,600]
                bpe_preact1 += bpe_x_ # [B,dunits*2]=[10,600]
                bpe_preact1 = torch.sigmoid(bpe_preact1)
                bpe_r1 = self._slice(bpe_preact1, 0, self.dunits) # reset gate [B,dunits]=[10,300]
                bpe_u1 = self._slice(bpe_preact1, 1, self.dunits) # update gate [B,dunits]=[10,300]
                bpe_preactx1 = self.bpe_rnn1_Ux(bpe_h_) # no bias [B,dunits]=[10,300]
                bpe_preactx1 *= bpe_r1
                bpe_preactx1 += bpe_xx_ 
                bpe_h1 = torch.tanh(bpe_preactx1)
                bpe_h1 = bpe_u1 * bpe_h_ + (1. - bpe_u1) * bpe_h1
                bpe_h1 = bpe_m_.unsqueeze(1) * bpe_h1 + (1. - bpe_m_).unsqueeze(1) * bpe_h_
                aux2_att_state.append(bpe_h1)

                ## 2. Attention : c(t)=AttentionContext(s'(t), enc_o) 
                # || dunits (prior_att_c[B,eprojs] prior_att_w[B,T])
                bpe_prior_att_c, bpe_prior_att_w = self.att_bpe[att_bpe_idx](bpe_enc, hlens_bpe, bpe_h1, bpe_prev_alpha)
                bpe_prior_att_weights.append(bpe_prior_att_w)
                bpe_prior_att_context.append(bpe_prior_att_c)
                
                ## 3. RNN : s(t)=RNN(c(t),s'(t)) 
                # || att_c(eprojs) z_infocomb_list(dunits)
                bpe_preact2 = self.bpe_rnn2_U_n1(bpe_h1) # [B,dunits*2]=[10,600]
                bpe_preact2 += self.bpe_rnn2_Wc(bpe_prior_att_c) # no bias [B,dunits*2]=[10,600]
                bpe_preact2 = torch.sigmoid(bpe_preact2)
                bpe_r2 = self._slice(bpe_preact2, 0, self.dunits) # [B,dunits]
                bpe_u2 = self._slice(bpe_preact2, 1, self.dunits) # [B,dunits]
                bpe_preactx2 = self.bpe_rnn2_Ux_n1(bpe_h1) # [B,dunits]=[10,300]
                bpe_preactx2 *= bpe_r2
                bpe_preactx2 += self.bpe_rnn2_Wcx(bpe_prior_att_c) # [B,dunits]=[10,300]
                bpe_h2 = torch.tanh(bpe_preactx2)
                bpe_h2 = bpe_u2 * bpe_h1 + (1. - bpe_u2) * bpe_h2
                bpe_h2 = bpe_m_.unsqueeze(1) * bpe_h2 + (1. - bpe_m_).unsqueeze(1) * bpe_h1 # [B,dunits]=[10,300]           

                ############################## this is interaction operation ##############################
                if PREPARE_USING_B2C == True:
                    ## (1) RNN : s'(t)=RNN(y(t-1),s(t-1)) 
                    c2b_x_  = state_char2bpe_[:,i,:]
                    c2b_xx_ = state_char2bpex[:,i,:]      
                    c2b_preact3 = self.char2bpe_rnn1_U(char2bpe_state_h_) # no bias -> [B,dunits*2]=[10,600]
                    c2b_preact3 += c2b_x_ # [B,dunits*2]=[10,600]
                    c2b_preact3 = torch.sigmoid(c2b_preact3)
                    c2b_r3 = self._slice(c2b_preact3, 0, self.dunits) # reset gate [B,dunits]=[10,300]
                    c2b_u3 = self._slice(c2b_preact3, 1, self.dunits) # update gate [B,dunits]=[10,300]
                    c2b_preactx3 = self.char2bpe_rnn1_Ux(char2bpe_state_h_) # no bias [B,dunits]=[10,300]
                    c2b_preactx3 *= c2b_r3
                    c2b_preactx3 += c2b_xx_ 
                    c2b_h3 = torch.tanh(c2b_preactx3)
                    c2b_h3 = c2b_u3 * char2bpe_state_h_ + (1. - c2b_u3) * c2b_h3
                    c2b_h3 = bpe_m_.unsqueeze(1) * c2b_h3 + (1. - bpe_m_).unsqueeze(1) * char2bpe_state_h_
                    ## (2) Attention : c(t)=AttentionContext(s'(t), enc_o) 
                    c2b_att_c, c2b_att_w = self.att_char2bpe[att_bpe_idx](bpe_enc, hlens_bpe, c2b_h3, prev_c2b_alpha)
                    c2b_att_weights.append(c2b_att_w)
                    c2b_att_context.append(c2b_att_c)
                    aux_att_state.append(c2b_h3)
                    ## (3) RNN : s(t)=RNN(c(t),s'(t)) 
                    hc = torch.sigmoid( self.c2b_statecomb(c2b_h3) ) + bpe_h1
                    h1h3_comb = torch.cat([bpe_h1, c2b_h3, hc],dim=1)
                    c2b_preact4 = self.char2bpe_rnn2_U_n1(h1h3_comb) # [B,dunits*2]=[10,600]
                    c2b_preact4 += self.char2bpe_rnn2_Wc(c2b_att_c) # no bias [B,dunits*2]=[10,600]
                    c2b_preact4 = torch.sigmoid(c2b_preact4)
                    c2b_r4 = self._slice(c2b_preact4, 0, self.dunits) # [B,dunits]
                    c2b_u4 = self._slice(c2b_preact4, 1, self.dunits) # [B,dunits]
                    c2b_preactx4 = self.char2bpe_rnn2_Ux_n1(h1h3_comb) # [B,dunits]=[10,300]
                    c2b_preactx4 *= c2b_r4
                    c2b_preactx4 += self.char2bpe_rnn2_Wcx(c2b_att_c) # [B,dunits]=[10,300]
                    c2b_h4 = torch.tanh(c2b_preactx4)
                    c2b_h4 = c2b_u4 * c2b_h3 + (1. - c2b_u4) * c2b_h4
                    c2b_h4 = bpe_m_.unsqueeze(1) * c2b_h4 + (1. - bpe_m_).unsqueeze(1) * c2b_h3 # [B,dunits]=[10,300]       
                    
                    ## (4) combine information for classification
                    context = bpe_prior_att_c # torch.cat([bpe_prior_att_c, c2b_att_c], dim=1)
                    cstates = torch.cat([bpe_h2, c2b_h4], dim=1)
                    logit_inter = self.classbpe_state_W(cstates) + self.classbpe_yprev_W(state_bpe[:, i, :]) + self.classbpe_enc_W(context) # [B,dunits]=[10,300]
                    logit_inter = torch.reshape( logit_inter, (batch, -1, 2) )
                    logit_inter, _ = torch.max( logit_inter, dim=-1 ) # [B,dunits//2]=[B,150]
                    probs = self.bpe_class_W(logit_inter) # [B,odim]=[10,496]
                    probs = F.softmax(probs, dim=-1) # [B,496]
                    bpe_output_all.append(probs)

                ## 7. update previous variables
                bpe_h_ = bpe_h2
                bpe_prev_alpha = bpe_prior_att_w
                prev_c2b_alpha = c2b_att_w
            ###### Codes for BPE decoder and the additional operation (finish) ######   

        ## 8. Get mainly loss : CE and auxiliary loss : KL-penalty
        ## 8.1 : char
        ypred_char = torch.stack(char_output_all, dim=1).view(batch * olength_char, self.odim_char)   # [B,decT,odim_char]
        ...
        if self.verbose > 0 and self.bpe_list is not None:
            ...
            for (i, y_hat), y_true in zip(enumerate(ys_hat.detach().cpu().numpy()), ys_true.detach().cpu().numpy()):
                ...
                logging.info("groundtruth[%d]: " % i + seq_true)
                logging.info("prediction [%d]: " % i + seq_hat)    
        # # 9.3(inter) pad_outputs: prediction tensors (B*decoder_T, D)
        # #            pad_targets: target tensors (B, decoder_T, D)
        # acc_inter = th_accuracy(ypred_inter, ys_bpe_out_pad, ignore_label=self.ignore_id)
        # logging.info('att_inter loss:' + ''.join(str(self.loss_inter.item()).split('\n')))
        ...
        if self.labeldist_bpe is not None: 
            ...
            #########################
            # loss_reg_inter = - torch.sum((torch.log(ypred_inter) * self.vlabeldist_bpe).view(-1), dim=0) / len(ys_bpe_in)
            # self.loss_inter = (1. - self.lsm_weight) * self.loss_inter + self.lsm_weight * loss_reg_inter

        ## Auxilitary Ctx loss start  ##
        c2b_att_context = torch.stack(c2b_att_context, dim=1)
        aux_att_state = torch.stack(aux_att_state, dim=1)
        ypred_logit_aux = (self.inter_class_state(aux_att_state) + self.inter_class_context(c2b_att_context))
        ypred_logit_aux = ypred_logit_aux.view(batch * olength_bpe, self.odim_bpe)
        ypred_aux = F.softmax(ypred_logit_aux, dim=-1)
        self.loss_aux = F.categorical_crossentropy(ypred_aux, ys_bpe_out_pad.contiguous().view(-1).long(), ignore_index=self.ignore_id, reduction='none')
        self.loss_aux = self.loss_aux.view(batch, olength_bpe) 
        self.loss_aux = self.loss_aux.sum(1)
        self.loss_aux = self.loss_aux.mean(0) # to be scalar   
        acc_aux = th_accuracy(ypred_aux, ys_bpe_out_pad, ignore_label=self.ignore_id)
        ## Auxilitary Ctx loss finish ##

        ## Auxilitary Ctx loss start  ##
        bpe_att_context = torch.stack(bpe_prior_att_context, dim=1)
        aux2_att_state = torch.stack(aux2_att_state, dim=1)
        ypred_logit_aux2 = (self.bpe_class_state(aux2_att_state) + self.bpe_class_context(bpe_att_context))
        ypred_logit_aux2 = ypred_logit_aux2.view(batch * olength_bpe, self.odim_bpe)
        ypred_aux2 = F.softmax(ypred_logit_aux2, dim=-1)
        self.loss_aux2 = F.categorical_crossentropy(ypred_aux2, ys_bpe_out_pad.contiguous().view(-1).long(), ignore_index=self.ignore_id, reduction='none')
        self.loss_aux2 = self.loss_aux2.view(batch, olength_bpe) 
        self.loss_aux2 = self.loss_aux2.sum(1)
        self.loss_aux2 = self.loss_aux2.mean(0) # to be scalar   
        # acc_aux2 = th_accuracy(ypred_aux2, ys_bpe_out_pad, ignore_label=self.ignore_id)
        ## Auxilitary Ctx loss finish ##

        auxilitary_entropy_loss = True
        if auxilitary_entropy_loss == True:
            ## calcualte entropy of attention score start  ##
            bpe_attention_score = torch.stack(bpe_prior_att_weights, dim=1) # BPE alpha score
            c2b_attention_score = torch.stack(c2b_att_weights, dim=1) # C2B alpha score
            bpe_entropy = torch.sum(-1 * bpe_attention_score * torch.log(bpe_attention_score + 1e-10), dim=-1) # (B,decT,T)->[B.decT]
            c2b_entropy = torch.sum(-1 * c2b_attention_score * torch.log(c2b_attention_score + 1e-10), dim=-1) # (B,decT,T)->[B.decT]
            if 0:
                ent_bpe = (bpe_entropy.sum(1).mean(0))
                ent_c2b = (c2b_entropy.sum(1).mean(0))
            else:
                bpelens = torch.tensor(ys_bpe_out_len).to(bpe_attention_score)
                ent_bpe = ((1.0/(bpelens) * (bpe_entropy*y_bpe_mask).sum(1)).to(bpe_attention_score)).mean(0)
                ent_c2b = ((1.0/(bpelens) * (c2b_entropy*y_bpe_mask).sum(1)).to(c2b_attention_score)).mean(0)
            ## calcualte entropy of attention score finish ##
        self.batchcount += 1
        return [self.loss_char, self.loss_bpe, self.loss_aux+self.loss_aux2], [acc_char, acc_bpe, acc_aux], [ent_bpe, ent_c2b]
    ...
    def calculate_all_attentions(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0):  #### change here
        ...
        att_ws_inter = att_to_numpy(c2b_att_weights, self.att_char2bpe[att_bpe_idx]) 
        att_ws_char = att_to_numpy(char_prior_att_weights, self.att_char[att_char_idx]) 
        att_ws_bpe = att_to_numpy(bpe_prior_att_weights, self.att_bpe[att_bpe_idx]) 
        return (att_ws_bpe, att_ws_inter) #  



    def char_onestep_decoder(self, exp_h, exp_hlens, prev_alpha, h_, yseq, strm_idx=0):
        ...
        logit_char = self.classchar_state_W(h2) + self.classchar_enc_W(prior_att_c) # [n_bb,dunits]
        probs = F.softmax(logit_char, dim=-1) # [n_bb,odim]

        ## eos score
        # eos_char_score = (torch.log(probs) * self.char_eosmask).sum(-1) # [n_bb,char_odim] -> [n_bb]

        return torch.log(probs)
    


    def char_onebpe_decoder(self, exp_h, exp_hlens, prev_alpha, h_, yseq, char_len, strm_idx=0):  # realmask查验
        ...
        for ii in range(int(max(char_len))):
            ...
            ## 2. Attention eqn.(23): c(t)=Attentit char
            ey = self.dropout_emb_char(self.embed_char(vy)) # [n_bb, embsize]
            xx_ = self.char_rnn1_Wx(ey) # state_belowx[n_bb, dunits]
            x_ = self.char_rnn1_W(ey) # [n_bb, 2*dunits]
            cc_ = exp_h # [n_bb, T, eprojs]
            ...
            local_scores += (torch.log(probs) * true_choose_mask).sum(1) * m_                                                   # [batch*beam,odim]->[batch*beam] 这里在处理完长度问题之后再和外界处理
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        print('[decoders_new_mltaskraux2] CHOICE_SCORE_TYPES is %s.' %(CHOICE_SCORE_TYPES))
        ...
        for i in six.moves.range(maxlen):
            if decode_choice == 'char':
                ...
                logging.debug('BPE position ' + str(i))
                ...
                ## 4. Do classification for every frame
                logit_bpe = self.classbpe_state_W(h2) + self.classbpe_enc_W(prior_att_c) # [n_bb,dunits]
                probs = F.softmax(logit_bpe, dim=1) # [n_bb,odim]
                ...
                ## 3. RNN : s(t)=RNN(c(t),s'(t)) 
                hc = torch.sigmoid( self.c2b_statecomb(h3) ) + h1
                h1h3_comb = torch.cat([h1, h3, hc],dim=1)
                preact4 = self.char2bpe_rnn2_U_n1(h1h3_comb) # [B,dunits*2]=[10,600]
                ...
                preactx4 = self.char2bpe_rnn2_Ux_n1(h1h3_comb) # [B,dunits]=[10,300]
                preactx4 *= r4
                ...
                logit_inter = self.classbpe_state_W(cstates) + self.classbpe_yprev_W(ey) + self.classbpe_enc_W(context) # [B,dunits]=[10,300]
                logit_inter = torch.reshape( logit_inter, (batch, -1, 2) )
                logit_inter, _ = torch.max( logit_inter, dim=-1 ) # [B,dunits//2]=[B,150]
                probs = self.bpe_class_W(logit_inter) # [B,odim]=[10,496]
                probs = F.softmax(probs, dim=-1) # [B,496]
        ...
        nbest_hyps = [sorted(ended_hyps[samp_i], key=lambda x: x['score'], reverse=True)[:min(len(ended_hyps[samp_i]), recog_args.nbest)] # recog_args.nbest (1)
                      for samp_i in six.moves.range(batch)]
        print('[c1v4_entropy] The value of CHOICE_SCORE_TYPES is %s. TJ-USING.' %(CHOICE_SCORE_TYPES))

        return nbest_hyps



def decoder_new_mltaskr_modification3_aux7c1v4_entropy_for(
    ...
    return MLSoftmaxDecoder(
                ...
                sos_char, eos_char, sos_bpe, eos_bpe, 
                att_char, att_bpe, att_char2bpe, 
                args, args.verbose, args.char_list, args.bpe_list, 
                labeldist_char, labeldist_bpe, 
