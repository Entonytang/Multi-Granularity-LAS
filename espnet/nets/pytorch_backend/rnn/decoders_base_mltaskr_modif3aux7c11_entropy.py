...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('[decoder_base_mltaskller_modification3_for] This is info transformation from char to BPE (mltaskbaserm3).')
        print('e2e_asrsA2 -> mltaskbaser_modi3a7s : aux7(7 aux file) + entropy + emb256 + emb256 + small')
        print('Deep Fusion classification.')
        print('-----------------------------------------------------------------------------------------------------------------')
        ...
        self.FCcontext = torch.nn.Linear(dunits, dunits)
        self.FCstate = torch.nn.Linear(dunits, dunits)
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        if SWITCH_BPE == True:
            ...
            for i in six.moves.range(olength_bpe):
                ...
                cstates = torch.cat([h2, torch.sigmoid(self.FCcontext(h2))*h4], dim=1)
                context = torch.cat([prior_att_c, torch.sigmoid(self.FCcontext(prior_att_c))*c2b_prior_att_c], dim=1)
...
def decoder_base_mltaskr_modification3_aux7c11_entropy_for(
