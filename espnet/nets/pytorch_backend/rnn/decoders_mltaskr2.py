...
JOINT_CHAR_DECODER = True ### SWITCH
...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('[decoders_mltaskr2.py] This is just information transformation from char to BPE (mltaskr2 [mltaski09r0]).')
        ...
        ## BPE RNN Layer 3
        self.bpe_rnn3_U_n1  = torch.nn.Linear(dunits, dunits*2)
        self.bpe_rnn3_Wc    = torch.nn.Linear(cprojs, dunits*2, bias=False)
        self.bpe_rnn3_Ux_n1 = torch.nn.Linear(dunits, dunits)
        self.bpe_rnn3_Wcx   = torch.nn.Linear(cprojs, dunits, bias=False)
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        if SWITCH_BPE == True:
            ...
            for i in six.moves.range(olength_bpe):
                ...
                ## Add. RNN : s(t)=RNN(c(t),s'(t)) 
                # || att_c(eprojs) z_infocomb_list(dunits)
                preact3 = self.bpe_rnn3_U_n1(hbpe_) # [B,dunits*2]=[10,600]
                preact3 += self.bpe_rnn3_Wc(h2) # no bias [B,dunits*2]=[10,600]
                preact3 = torch.sigmoid(preact3)
                r3 = self._slice(preact3, 0, self.dunits) # [B,dunits]
                u3 = self._slice(preact3, 1, self.dunits) # [B,dunits]
                preactx3 = self.bpe_rnn3_Ux_n1(hbpe_) # [B,dunits]=[10,300]
                preactx3 *= r3
                preactx3 += self.bpe_rnn3_Wcx(h2) # [B,dunits]=[10,300]
                h3 = torch.tanh(preactx3)
                h3 = u3 * h2 + (1. - u3) * h3
                h3 = m_.unsqueeze(1) * h3 + (1. - m_).unsqueeze(1) * h2 # [B,dunits]=[10,300]

                ## 4. Classification for every frames
                # h2[B,dunits]  state_bpe[B,decT]  prior_att_c[B,dunits//2]
                logit_all_for_classif = self.classbpe_state_W(h3) + self.classbpe_yprev_W(state_bpe[:, i, :]) + self.classbpe_enc_W(prior_att_c) # [B,dunits]=[10,300]
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        if (JOINT_CHAR_DECODER == True):
            ...
            if (JOINT_CHAR_DECODER == True): self.att_char[att_char_idx].reset()
            ...
            if i > minlen:
                ...
                for samp_i in six.moves.range(batch): # 对于其中每一个batch进行修改
                    ...
                    for beam_j in six.moves.range(beam):
                        if 0: # (JOINT_CHAR_DECODER == True): 
                            if eos_vscores[samp_i, beam_j] > thr[samp_i]:
                                yk = y_prev[k][:]
                                yk.append(eos_token)
                                yk_char = y_prev_char[k][:] #########
                                yk_char.append(self.eos_char)
                                if len(yk) < hlens[samp_i]:
                                    # print('saved result',eos_vscores_char)
                                    _vscore = eos_vscores[samp_i][beam_j] + penalty_i
                                    _vscore_char = eos_vscores_char[samp_i][beam_j]+ penalty_i
                                    _score = _vscore.data.cpu().numpy()
                                    _score_char = _vscore_char.data.cpu().numpy()
                                    normalize_score = False
                                    if normalize_score:
                                        _vscore = _vscore / len(yk)
                                        _vscore_char = _vscore_char / len(yk_char)
                                    _score = _vscore.data.cpu().numpy()
                                    _score_char = _vscore_char.data.cpu().numpy() # 
                                    ended_hyps[samp_i].append({'yseq': yk, 'yseq_char': yk_char, 'vscore': _vscore, 'score': _score, 'char_score': _score_char, 'bpe_score': _score}) ## 
                            k = k + 1
                        else:
                            if eos_vscores[samp_i, beam_j] > thr[samp_i]:
                                yk = y_prev[k][:]
    ...
    # Right Coding for decoding process
