...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('Remove ypred in classification (both CHAR and BPE).：')
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        if SWITCH_BPE == True:
            ...
            for i in six.moves.range(olength_bpe):
                ...
                logit_all_for_classif = self.classbpe_state_W(cstates) + self.classbpe_enc_W(context) # [B,dunits]=[10,300]
    ...
    def char_onebpe_training(self, exp_h, exp_hlens, prev_alpha, h_, yseq, char_len, char_output_all, charposterindex, strm_idx=0):  # realmask
        ...
        for ii in range(int(max(char_len))):
            ...
            logit_all_for_classif = self.classchar_state_W(h2) + self.classchar_enc_W(prior_att_c) # [nbatch,dunits]
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        for i in six.moves.range(maxlen):
            if decode_choice == 'char':
                ...
                logit_all_for_classif = self.classchar_state_W(h2) + self.classchar_enc_W(prior_att_c) # [n_bb,dunits]
                similarb = logit_all_for_classif.size(0) # 
                logit_linear    = torch.reshape( logit_all_for_classif, (similarb, self.dunits//2, 2) )
                ...
                logit_all_for_classif = self.classbpe_state_W(cstates) + self.classbpe_enc_W(context) # [n_bb,dunits]
                similarb = logit_all_for_classif.size(0) # 
...
def decoder_base_mltaskller_modification3_simple_for(
