...
traditional_loss_calculate = False ## check here
decode_choice = "bpe"
...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('embsize_bpe is %d while embsize_char is %d.' %(embsize_bpe, embsize_char))
        print('[decoders_mltaskn_entropy.py] This is no information transformation from char to BPE (mtask).')
        ...
        self.classbpe_state_W = torch.nn.Linear(dunits, dunits)
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        if SWITCH_CHAR == True:
           ...
           # char y-label length
            ...
            h_ = self.mean_init_char_state(char_enc, hlens_char) # hlens_char ?
            prev_postr_energy = (to_device(self, make_trueframe_mask(hlens_char))).float() # previous postr attention score
            ...
            for i in six.moves.range(olength_char):
                m_  = y_char_mask[:,i] # the variables for this char decoder steps
                x_  = state_char_[:,i,:]
                xx_ = state_charx[:,i,:]
                cc_ = char_enc # [B,decT,dunit//2]

                ## 1. RNN : s'(t)=RNN(y(t-1),s(t-1)) 
                # || to combine [emb(t-1),state(t-1)] -> predt-state(t) 
                # || RNN(dunits) : concat[ey,ctx-previous-posterior]
                preact1 = self.char_rnn1_U(h_) # no bias -> [B,dunits*2]=[10,600]
                ...
                preactx1 = self.char_rnn1_Ux(h_) # no bias [B,dunits]=[10,300]
                preactx1 *= r1
                preactx1 += xx_ 
                ...
                prior_att_c, prior_att_w = self.att_char[att_char_idx](cc_, hlens_char, h1, prev_alpha)
                ...
                preact2 = self.char_rnn2_U_n1(h1) # [B,dunits*2]=[10,600]
                preact2 += self.char_rnn2_Wc(prior_att_c) # no bias [B,dunits*2]=[10,600]
                preact2 = torch.sigmoid(preact2)
                r2 = self._slice(preact2, 0, self.dunits) # [B,dunits]
                u2 = self._slice(preact2, 1, self.dunits) # [B,dunits]
                preactx2 = self.char_rnn2_Ux_n1(h1) # [B,dunits]=[10,300]
                preactx2 *= r2
                preactx2 += self.char_rnn2_Wcx(prior_att_c) # [B,dunits]=[10,300]
                ...
                logit_all_for_classif = self.classchar_state_W(h2) + self.classchar_yprev_W(state_char[:, i, :]) + self.classchar_enc_W(prior_att_c) # [B,dunits]=[10,300]
                logit_linear = torch.reshape( logit_all_for_classif, (batch, self.dunits//2, 2) )
                logit_clf, _ = torch.max( logit_linear, dim=-1 ) # [B,dunits//2]=[B,150]
                probs = self.char_class_W(logit_clf) # [B,odim]=[10,496]
                probs = F.softmax(probs, dim=-1) # [B,496]

                ## 5. save posterior output
                char_output_all.append(probs)
                
                ## 6. update previous variables
                h_ = h2
                prev_alpha = prior_att_w
            ###### Codes for CHAR decoder and the additional operation (finish) ######
        ...
        if SWITCH_BPE == True:
           ...
           # bpe y-label length
            ...
            for i in six.moves.range(olength_bpe):
                ...
                logit_all_for_classif = self.classbpe_state_W(h2) + self.classbpe_yprev_W(state_bpe[:, i, :]) + self.classbpe_enc_W(prior_att_c) # [B,dunits]=[10,300]
                logit_linear = torch.reshape( logit_all_for_classif, (batch, self.dunits//2, 2) )
                logit_clf, _ = torch.max( logit_linear, dim=-1 ) # [B,dunits//2]=[B,150]
        ...
        return [self.loss_char, self.loss_bpe, self.loss_bpe], [acc_char, acc_bpe, acc_bpe], [ent_bpe, ent_bpe]
    ...
    def calculate_all_attentions(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0):  #### change here
        ...
        if SWITCH_CHAR == True:
           ...
           # char y-label length
            ...
            # padding for ys with -1
            # pys: utt x olen (char)
            ys_char_in_pad  = pad_list(ys_char_in, self.eos_char) # pad feature torch.Tensor([B,decT])
            ys_char_out_pad = pad_list(ys_char_out, self.ignore_id) # pad output symbol
            ...
            for i in six.moves.range(olength_char):
                ...
                ## 4. Classification for every frames
                # h2[B,dunits]  state[B,decT]  prior_att_c[B,dunits//2]
                logit_all_for_classif = self.classchar_state_W(h2) + self.classchar_yprev_W(state_char[:, i, :]) + self.classchar_enc_W(prior_att_c) # [B,dunits]=[10,300]
                logit_linear = torch.reshape( logit_all_for_classif, (batch, self.dunits//2, 2) )
                logit_clf, _ = torch.max( logit_linear, dim=-1 ) # [B,dunits//2]=[B,150]
                probs = self.char_class_W(logit_clf) # [B,odim]=[10,496]
        ...
        if SWITCH_BPE == True:
           ...
           # bpe y-label length
            ...
            # padding for ys with -1
            # pys: utt x olen (bpe)
            ys_bpe_in_pad  = pad_list(ys_bpe_in, self.eos_bpe) # pad feature torch.Tensor([B,decT])
            ys_bpe_out_pad = pad_list(ys_bpe_out, self.ignore_id) # pad output symbol
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        for i in six.moves.range(maxlen):
            if decode_choice == 'char':
                ...
                logging.debug('Split[mltask] BPE position ' + str(i))
                ...
                ## 4. Do classification for every frame
                logit_all_for_classif = self.classbpe_state_W(h2) + self.classbpe_yprev_W(ey) + self.classbpe_enc_W(prior_att_c) # [n_bb,dunits]
                similarb = logit_all_for_classif.size(0) # 
                logit_linear    = torch.reshape( logit_all_for_classif, (similarb, self.dunits//2, 2) )
                logit_clf, _ = torch.max( logit_linear, dim=2 ) # [n_bb,dunits//2]
        ...
        nbest_hyps = [sorted(ended_hyps[samp_i], key=lambda x: x['score'], reverse=True)[:min(len(ended_hyps[samp_i]), recog_args.nbest)] # recog_args.nbest (1)
                      for samp_i in six.moves.range(batch)]
        print("72ERROR : decoder_base_mltaskn_for", BPE_JointDecoding)
    ...
    def char_onebpe_decoder(self, exp_h, exp_hlens, prev_alpha, h_, yseq, char_len, strm_idx=0):  # realmask查验
        ...
        for ii in range(int(max(char_len))):
            ...
            local_scores += (torch.log(probs) * true_choose_mask).sum(1) * m_                                                   # [batch*beam,odim]->[batch*beam] 处理完长度后再和外界处理
...
def decoder_base_mltaskn_entropy_for(
