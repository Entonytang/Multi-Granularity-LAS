...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('[decoder_base_mltasker_mod3_aux7r8_entropy_for] This is info transformation from char to BPE (mltaskbaser_modi3a7r8).')
        print('aux7r8 : 7 aux file + entropy + r8.')
        print('r8: we use the char_ctx_ in .c2b_rnn1 对应着r8的WSJ版本！')
        print('v5: RNN3(没有GLU辅助输入)额外添加上char_ctx_同时作为辅助输入，在原来GLU RNN3输入融合的基础上[主h3]。')
        print('-----------------------------------------------------------------------------------------------------------------')
        ## Deep Fusion Like FC 
        self.c2b_statecomb = torch.nn.Linear(dunits, dunits)
        self.c2b_ctxcomb = torch.nn.Linear(eprojs, eprojs)    
        ...
        self.char2bpe_rnn1_WxA = torch.nn.Linear(self.embsize_bpe, dunits)
        self.char2bpe_rnn1_WA  = torch.nn.Linear(self.embsize_bpe, dunits*2)
        self.char2bpe_rnn1_Wx = torch.nn.Linear(dunits, dunits)
        self.char2bpe_rnn1_W  = torch.nn.Linear(dunits, dunits*2)
        ...
        self.char2bpe_rnn1_WxB= torch.nn.Linear(dunits, dunits, bias=False)
        self.char2bpe_rnn1_WB = torch.nn.Linear(dunits, dunits*2, bias=False)
        ...
        self.char2bpe_rnn2_Wc    = torch.nn.Linear(2*cprojs, dunits*2, bias=False)
        self.char2bpe_rnn2_Wcx   = torch.nn.Linear(2*cprojs, dunits, bias=False)
        ...
        #########################
        self.FCaux_class_state = torch.nn.Linear(dunits, odim_bpe, bias=False)
        self.FCaux_class_context = torch.nn.Linear(dunits, odim_bpe)
        #########################

        #########################
        self.FCaux2_class_state = torch.nn.Linear(dunits, odim_bpe, bias=False)
        self.FCaux2_class_context = torch.nn.Linear(dunits, odim_bpe)
        #########################
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        if SWITCH_BPE == True:
            ...
            char_ctx_ = (subprev_alpha.unsqueeze(2) * hs_pad).sum(1)  
            ...
            state_char2bpe_ = self.char2bpe_rnn1_WA(eys_bpe)  # [B,decT,dunits]=[10,144,600]
            state_char2bpex = self.char2bpe_rnn1_WxA(eys_bpe) # [B,decT,dunits]=[10,144,300]
            for i in six.moves.range(olength_bpe):
                ...
                auxilitary_char2bpe_inter_part = True
                if auxilitary_char2bpe_inter_part == True:
                    c2b_x_  = state_char2bpe_[:,i,:]
                    c2b_xx_ = state_char2bpex[:,i,:]
                    ## 1. RNN : s'(t)=RNN(y(t-1),s(t-1)) 
                    # || to combine [emb(t-1),state(t-1)] -> predt-state(t) 
                    # || RNN(dunits) : concat[ey,ctx-previous-posterior]
                    preact3 = self.char2bpe_rnn1_U(inter_h_) # no bias -> [B,dunits*2]=[10,600]
                    preact3 += self.char2bpe_rnn1_W(char_ctx_)
                    preact3 += self.char2bpe_rnn1_WB(hbpe_)
                    ...
                    preactx3 *= r3
                    preactx3 += self.char2bpe_rnn1_Wx(char_ctx_)
                    preactx3 += self.char2bpe_rnn1_WxB(hbpe_) 
                    preactx3 += c2b_xx_ 
                    h3 = torch.tanh(preactx3)
                    h3 = u3 * inter_h_ + (1. - u3) * h3
                    h3 = m_.unsqueeze(1) * h3 + (1. - m_).unsqueeze(1) * inter_h_
                    ...
                    aux_att_state.append(h3)

                    ## 3. RNN : s(t)=RNN(c(t),s'(t)) 
                    # || att_c(eprojs) z_infocomb_list(dunits)
                    h1h3comb = torch.sigmoid( self.c2b_statecomb(h1) ) * h3
                    h13comb = torch.cat([h3, h1h3comb], dim=1)
                    ############
                    ctxcomb = torch.sigmoid( self.c2b_ctxcomb(prior_att_c) ) * c2b_prior_att_c
                    hctxcomb = torch.cat([c2b_prior_att_c, ctxcomb], dim=1)
                    ############
                    preact4 = self.char2bpe_rnn2_U_n1(h13comb) # [B,dunits*2]=[10,600]
                    preact4 += self.char2bpe_rnn2_Wc(hctxcomb) # no bias [B,dunits*2]=[10,600]
                    preact4 = torch.sigmoid(preact4)
                    r4 = self._slice(preact4, 0, self.dunits) # [B,dunits]
                    u4 = self._slice(preact4, 1, self.dunits) # [B,dunits]
                    preactx4 = self.char2bpe_rnn2_Ux_n1(h13comb) # [B,dunits]=[10,300]
                    preactx4 *= r4
                    preactx4 += self.char2bpe_rnn2_Wcx(hctxcomb) # [B,dunits]=[10,300]
                    h4 = torch.tanh(preactx4)
                    h4 = u4 * h3 + (1. - u4) * h4
                    h4 = m_.unsqueeze(1) * h4 + (1. - m_).unsqueeze(1) * h3 # [B,dunits]=[10,300]

                    ## Information combine for classification
                    context = prior_att_c # context = torch.cat([prior_att_c, c2b_prior_att_c],dim=1)
                ...
                if decode_choice == "bpe":
                    ...
                    ysequence = self.eos_char * np.ones((nbatch, int(max_len+1) ), dtype=np.int)
                    ...
                    subprev_alpha, hbpe_, char_output_all, charposterindex, char_ctx_ = self.char_onebpe_training(bpe_enc, hlens_bpe, subprev_alpha, hbpe_, ysequence, char_len[i,:], char_output_all, charposterindex, char_ctx_) # char_len[i,:] is batch
                ...
                h_ = h2                           # state
                inter_h_ = h4                     # state
                prev_alpha = prior_att_w          # alpha score
                prev_c2b_alpha = c2b_prior_att_w  # alpha score
        ...
        if auxilitary_entropy_loss == True:
            ...
            c2b_att_context = torch.stack(c2b_prior_att_context, dim=1)
            aux_att_state = torch.stack(aux_att_state, dim=1)
            ypred_logit_aux = (self.FCaux_class_state(aux_att_state) + self.FCaux_class_context(c2b_att_context))
            ...
            ypred_logit_aux2 = (self.FCaux2_class_state(aux2_att_state) + self.FCaux2_class_context(bpe_att_context))
            ...
            c2b_attention_score = torch.stack(c2b_prior_att_weights, dim=1) # C2B alpha score
    ...
    def char_onebpe_training(self, exp_h, exp_hlens, prev_alpha, h_, yseq, char_len, char_output_all, charposterindex, char_ctx_, strm_idx=0):  # realmask
        ...
        for ii in range(int(max(char_len))):
            ...
            char_ctx_ = m_.unsqueeze(1) * prior_att_c + (1. - m_).unsqueeze(1) * char_ctx_
        ...
        return prev_alpha, h_, char_output_all, charposterindex, char_ctx_
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        elif (CHOICE_SCORE_TYPES == "bpe1char") or (CHOICE_SCORE_TYPES == "bpe2char"):
        ...
        if (decode_choice == 'char'):
            ...
            subprev_alpha = ( subprev_energy / (subprev_energy.sum(1, keepdim=True)+1e-8) )                         # [n_bb, ]           
            char_ctx_ = (subprev_alpha.unsqueeze(2) * exp_h).sum(1)  
        ...
        for i in six.moves.range(maxlen):
            if decode_choice == 'char':
                ...
                auxilitary_char2bpe_inter_part = True
                if auxilitary_char2bpe_inter_part == True:
                    ## 0. previous state combine
                    info = torch.sigmoid( self.c2b_statecomb2(inter_h_) ) + hbpe_

                    ## 1. RNN : s'(t)=RNN(y(t-1),s(t-1)) 
                    # || to combine [emb(t-1),state(t-1)] -> predt-state(t) 
                    # || RNN(dunits) : concat[ey,ctx-previous-posterior]
                    preact3 = self.char2bpe_rnn1_U(info) # no bias -> [B,dunits*2]=[10,600]
                    preact3 += self.char2bpe_rnn1_W(char_ctx_)
                    preact3 += self.char2bpe_rnn1_WB(inter_h_)
                    preact3 += c2b_x_ # [B,dunits*2]=[10,600]
                    preact3 = torch.sigmoid(preact3)
                    r3 = self._slice(preact3, 0, self.dunits) # reset gate [B,dunits]=[10,300]
                    u3 = self._slice(preact3, 1, self.dunits) # update gate [B,dunits]=[10,300]
                    preactx3 = self.char2bpe_rnn1_Ux(info) # no bias [B,dunits]=[10,300]
                    preactx3 *= r3
                    preactx3 += self.char2bpe_rnn1_Wx(char_ctx_) 
                    preactx3 += self.char2bpe_rnn1_WxB(inter_h_) 
                    preactx3 += c2b_xx_ 
                    h3 = torch.tanh(preactx3)
                    h3 = u3 * hbpe_ + (1. - u3) * h3
                    ...
                    ## 3. RNN : s(t)=RNN(c(t),s'(t)) 
                    # || att_c(eprojs) z_infocomb_list(dunits)
                    h1h3comb = torch.sigmoid( self.c2b_statecomb1(h1) ) * h3
                    h1h3comb = torch.cat([h1, h1h3comb], dim=1)
                    preact4 = self.char2bpe_rnn2_U_n1(h1h3comb) # [B,dunits*2]=[10,600]
                    preact4 += self.char2bpe_rnn2_Wc(c2b_prior_att_c) # no bias [B,dunits*2]=[10,600]
                    preact4 = torch.sigmoid(preact4)
                    r4 = self._slice(preact4, 0, self.dunits) # [B,dunits]
                    u4 = self._slice(preact4, 1, self.dunits) # [B,dunits]
                    preactx4 = self.char2bpe_rnn2_Ux_n1(h1h3comb) # [B,dunits]=[10,300]
                    preactx4 *= r4
            ...
            if (decode_choice == 'bpe'): 
                ...
                local_scores_char, subprev_alpha, hbpe_, char_ctx_ = self.char_onebpe_decoder(exp_h, exp_hlens, subprev_alpha, hbpe_, ysequence, char_len, char_ctx_) # yseq_char
        ...
        nbest_hyps = [sorted(ended_hyps[samp_i], key=lambda x: x['score'], reverse=True)[:min(len(ended_hyps[samp_i]), recog_args.nbest)] # recog_args.nbest (1)
                      for samp_i in six.moves.range(batch)]
        print('[mltaskraux7r8v5] The value of CHOICE_SCORE_TYPES is %s. TJ-USING.' %(CHOICE_SCORE_TYPES))
        print('              normalize_score is', normalize_score)

        return nbest_hyps



    def char_onebpe_decoder(self, exp_h, exp_hlens, prev_alpha, h_, yseq, char_len, char_ctx_, strm_idx=0):  # realmask查验
        # exp_h         ：torch.Tensor (beam,decT,dunit)
        # exp_hlens     ：list
        # previous_alpha：torch.Tensor (beam,decT)
        # previous_state：torch.Tensor (beam,dunit)
        ...
        return local_scores, prev_alpha, h_, char_ctx_
...
def decoder_base_mltasker_mod3_aux7r8v5_entropy_for(
