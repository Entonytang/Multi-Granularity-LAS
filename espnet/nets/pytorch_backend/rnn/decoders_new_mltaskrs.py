...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
            ...
            embsize_char=256, embsize_bpe=1024): ## RIGHT(28-28)
        ...
        print('MLTASKRN-NewBaseline :->: The decoder version is multi-level char&bpe (code checking -> second joint decoding).')
        print('[embsize] The BPE embsize of mltaskr is %d; The char embsize of mltaskr is %d.' %(embsize_bpe, embsize_char))
        # print('Setting BPE_JointDecoding', BPE_JointDecoding)
        print('This is just the comparable baseline [BPE & CHAR]. mltask.(Deep Fusion :  v10rd... [h1;sigmoid(FC(h1))*h3])')
        print('[decoders_new_mltaskrn.py] This is just information transformation from CHAR to BPE (mtaskrn).')
        ...
        self.c2b_rnn1_Wx = torch.nn.Linear(self.embsize_bpe, dunits)
        self.c2b_rnn1_W  = torch.nn.Linear(self.embsize_bpe, dunits*2)
    ...
    def calculate_all_attentions(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0):  #### change here
        ...
        if PREPARE_USING_B2C == True:
            ...
            for trans_idx, yseqs in enumerate(ys_bpe_all): # yseqs list(batch)  (decT(*),B)
                ...
                for idx,bpe_token in enumerate(bpe_token_list):
                    ...
                    if token == '<NOISE>' or token == '<*IN*>' or token == '<*MR.*>' or token == '<eos>' or token == '<blank>' or token == '<unk>': #   <unk> <blank> <eos>
                        yseq_char_inbpe[trans_idx][idx].append(self.inverse_chardict[token]) ## all length
                    else:
                        for iidx, str_ in enumerate(token):
                            yseq_char_inbpe[trans_idx][idx].append(self.inverse_chardict[str_]) ## all length
        ...
        if SWITCH_CHAR == True: ## CHAR 序列一次性全部做完
            ...
            ## transform the information csum_charlen is (44,2)
            charstate_store = torch.stack(charstate_storing, dim=1) # .view(batch, olength_char, self.dunits)
            # cindex = torch.clamp((csum_charlen[37,:]), min=0, max=LONGEST_LENS).unsqueeze(1) # [100,101]
            # cindex = torch.clamp((csum_charlen[38,:]), min=0, max=LONGEST_LENS).unsqueeze(1) # [104,104]
            # cindex = torch.clamp((csum_charlen[39,:]), min=0, max=LONGEST_LENS).unsqueeze(1) # [107,105]
            # cindex = torch.clamp((csum_charlen[40,:]), min=0, max=LONGEST_LENS).unsqueeze(1) # [108,107]
            # cindex = torch.clamp((csum_charlen[41,:]), min=0, max=LONGEST_LENS).unsqueeze(1) # [110,110]
            # cindex = torch.clamp((csum_charlen[42,:]), min=0, max=LONGEST_LENS).unsqueeze(1) # [111,111]
            # cindex = torch.clamp((csum_charlen[43,:]), min=0, max=LONGEST_LENS).unsqueeze(1) # [112,111]
