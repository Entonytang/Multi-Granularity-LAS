...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
                    ...
                    embsize_char=256, embsize_bpe=1024, lambdacharloss=0.2): ## RIGHT(28-28)
        ...
        if switch_addcode_forbi == True:
            self.b2c_statecomb = torch.nn.Linear(dunits, dunits) ## GLU
            self.char_rnn0_Wx    = torch.nn.Linear(self.embsize_char, dunits) ## additional RNN
            self.char_rnn0_W     = torch.nn.Linear(self.embsize_char, dunits*2)
            self.char_rnn0_U     = torch.nn.Linear(dunits, dunits*2, bias=False)
            self.char_rnn0_Ux    = torch.nn.Linear(dunits, dunits, bias=False)
            self.classchar_addit_W = torch.nn.Linear(dunits, dunits, bias=False) ## Additional Classification                   
        ###### New Parameters added for Bidirection (char->BPE && BPE->char finish) ######

        self.lambdacharloss = lambdacharloss
        if switch_addcode_forbi == True: # False is same as mltaskrll / True is BIDIR (mltaskrll + d)
            print('[decoders_base_mltaskrlld.py] This is just information transformation from char to BPE (mltaskbaserlld).')
        ...
        print('Deep Fusion :  v10rd... [h1;sigmoid(FC(h1))*h3] + 1F.')
        print('The embsize_bpe is %d while embsize_char is %d (lambda char loss %f).' %(embsize_bpe, embsize_char, self.lambdacharloss))
        ...
        self.bpeclass_dunits = 1024 # 1024 #### (self.dunits)
        print("The bpeclass_dunits for classification dimension is %d." %(self.bpeclass_dunits))
    ...
    def forward(self, hs_pad, hlens, ys_pad_char, ys_pad_bpe, strm_idx=0): 
        ...
        return [self.loss_char*self.lambdacharloss, self.loss_bpe], [acc_char, acc_bpe]
    ...
    def char_onebpe_training(self, exp_h, exp_hlens, prev_alpha, h_, yseq, char_len, char_output_all, charposterindex, bpe2char_state=None, strm_idx=0):  # realmask
        ...
        for ii in range(int(max(char_len))):
            ...
            if switch_addcode_forbi == False: 
                ...
                b2c_xx_ = self.char_rnn0_Wx(ey) # state_belowx[nbatch, dunits]
                b2c_x_ = self.char_rnn0_W(ey) # [nbatch, 2*dunits]                                 
                hc = torch.sigmoid( self.b2c_statecomb(h2) ) * bpe2char_state
                preact0 = self.char_rnn0_U(hc) # [nbatch,dunits*2]
                preact0 += b2c_x_ # [nbatch,dunits*2]
                preact0 = torch.sigmoid(preact0)
                r0 = self._slice(preact0, 0, self.dunits) # reset gate [nbatch,dunits]
                u0 = self._slice(preact0, 1, self.dunits) # update gate [nbatch,dunits]
                preactx0 = self.char_rnn0_Ux(hc) # [nbatch,dunits]
                preactx0 *= r0
                preactx0 += b2c_xx_ 
                h0 = torch.tanh(preactx0)
                h0 = u0 * hc + (1. - u0) * h0
                h0 = m_.unsqueeze(1) * h0 + (1. - m_).unsqueeze(1) * hc
                c2b_state = self.classchar_addit_W(h0)
                logit_all_for_classif = c2b_state + self.classchar_state_W(h2) + self.classchar_yprev_W(ey) + self.classchar_enc_W(prior_att_c) # [nbatch,dunits]
            ## ## ## ##
    ...
    def char_onebpe_decoder(self, exp_h, exp_hlens, prev_alpha, h_, yseq, char_len, bpe2char_state=None, strm_idx=0):  # realmask查验
        ...
        for ii in range(int(max(char_len))):
            ...
            if switch_addcode_forbi == False: 
                ...
                h0 = m_.unsqueeze(1) * h0 + (1. - m_).unsqueeze(1) * hc
                c2b_state = self.classchar_addit_W(h0)
                logit_all_for_classif = c2b_state + self.classchar_state_W(h2) + self.classchar_yprev_W(ey) + self.classchar_enc_W(prior_att_c) # [n_bb,dunits]                
            ## ## ## ##
            similarb = logit_all_for_classif.size(0)                                                                            # 
            logit_linear    = torch.reshape( logit_all_for_classif, (similarb, self.dunits//2, 2) )
            logit_clf, _ = torch.max( logit_linear, dim=2 )                                                                     # [n_bb,dunits//2]
    ...
    def char_onestep_decoder(self, exp_h, exp_hlens, prev_alpha, h_, yseq, bpe2char_state=None, strm_idx=0):
        ...
        if switch_addcode_forbi == False: 
            ...
            h0 = u0 * hc + (1. - u0) * h0
            c2b_state = self.classchar_addit_W(h0)
            logit_all_for_classif = self.classchar_state_W(h2) + self.classchar_yprev_W(ey) + self.classchar_enc_W(prior_att_c) + c2b_state # [n_bb,dunits]
...
def decoder_base_mltaskllerd_for(
