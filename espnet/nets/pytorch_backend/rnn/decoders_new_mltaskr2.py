...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('This is just the comparable baseline [BPE & CHAR]. mltask.(decoders_new_mltaskr2.py-decoder_new_mltaskr2_for)')
        print('Deep Fusion :  v10rd... [h1;sigmoid(FC(h1))*h3].')
        print('[decoders_new_mltaskr2.py] This is just information transformation from CHAR to BPE (mtaskrn).')
        ...
        self.c2b_rnn1_U  = torch.nn.Linear(2*dunits, dunits*2, bias=False)
        self.c2b_rnn1_Ux = torch.nn.Linear(2*dunits, dunits, bias=False)
        ## CHAR2BPE RNN Layer 2
        self.c2b_rnn2_U_n1  = torch.nn.Linear(dunits, dunits*2)
        self.c2b_rnn2_Wc    = torch.nn.Linear(cprojs, dunits*2, bias=False)
        self.c2b_rnn2_Ux_n1 = torch.nn.Linear(dunits, dunits)
        ...
        if SWITCH_BPE == True: ## 在这一组中采用index 来挑选所需要使用的(t-1) char state 
            ...
            for i in six.moves.range(olength_bpe):
                ...
                if PREPARE_USING_B2C == True:
                    ...
                    hcomb = torch.sigmoid( self.c2b_statecomb(bpe_h1) ) * char2bpe_state_h_
                    statecomb = torch.cat([bpe_h1, hcomb], dim=1)

                    c2b_preact3 = self.c2b_rnn1_U(statecomb) # no bias -> [B,dunits*2]=[10,600]
                    ...
                    c2b_preactx3 = self.c2b_rnn1_Ux(statecomb) # no bias [B,dunits]=[10,300]
                    c2b_preactx3 *= c2b_r3
                    ...
                    c2b_preact4 = self.c2b_rnn2_U_n1(c2b_h3) # [B,dunits*2]=[10,600]
                    ...
                    c2b_preactx4 = self.c2b_rnn2_Ux_n1(c2b_h3) # [B,dunits]=[10,300]
                    c2b_preactx4 *= c2b_r4
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, bpe_list=None, rnnlm=None, normalize_score=True, strm_idx=0):
        ...
        for i in six.moves.range(maxlen):
            if decode_choice == 'char':
                ...
                hcomb = torch.sigmoid( self.c2b_statecomb(h1) ) * hbpe_
                statecomb = torch.cat([h1, hcomb], dim=1)                

                preact3 = self.c2b_rnn1_U(statecomb) # no bias -> [B,dunits*2]=[10,600]
                ...
                preactx3 = self.c2b_rnn1_Ux(statecomb) # no bias [B,dunits]=[10,300]
                preactx3 *= r3
                ...
                ## 3. RNN : s(t)=RNN(c(t),s'(t))
                preact4 = self.c2b_rnn2_U_n1(h3) # [B,dunits*2]=[10,600]
                ...
                preactx4 = self.c2b_rnn2_Ux_n1(h3) # [B,dunits]=[10,300]
                preactx4 *= r4
