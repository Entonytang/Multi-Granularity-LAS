...
class MLSoftmaxDecoder(torch.nn.Module):
    def __init__(self, eprojs, dtype, dlayers, dunits, 
        ...
        print('[decoders_mltaskr.py] This is just information transformation from char to BPE (mtaskrl).')
        ...
        self.classbpe_state_W = torch.nn.Linear(2*dunits, 2*dunits)
        self.classbpe_yprev_W = torch.nn.Linear(self.embsize_bpe, 2*dunits, bias=False)
        self.classbpe_enc_W   = torch.nn.Linear(2*cprojs, 2*dunits, bias=False)
        self.bpe_class_W = torch.nn.Linear(dunits, odim_bpe)
        ## Constant value
