...
class PlotAttentionReport(extension.Extension):
    ...
    def __call__(self, trainer):
        ...
        if isinstance(att_ws, list):  # multi-encoder case
        ...
        elif (type(att_ws) == tuple) and len(att_ws) == 2:
            ###########################################################
            if os.path.exists( self.outdir+'/attention_visual.mat' ):
                visualize_alpha = scio.loadmat(self.outdir+'/attention_visual.mat')
            else: # os.mkdir(destname.split("/")[0]) 
                visualize_alpha = {}
            ###########################################################

            [bpe_att_ws, aux_att_ws] = att_ws
            for idx, att_w in enumerate(bpe_att_ws): # for each sentence
                filename = "%s/%s.bpe.ep.{.updater.epoch}.png" % (self.outdir, self.data[idx][0]) # att_w is (dec_T,T)
                ###########################################################
                name = "bpe_%s_epoch{.updater.epoch}" % (self.data[idx][0]) 
                name = name.format(trainer)
                visualize_alpha[name] = np.squeeze(att_w)
                ###########################################################                
                att_w = self.get_attention_weight(idx, att_w)
                self._plot_and_save_attention(att_w, filename.format(trainer))
            for idx, att_w in enumerate(aux_att_ws): # for each sentence
                filename = "%s/%s.aux.ep.{.updater.epoch}.png" % (self.outdir, self.data[idx][0]) # att_w is (dec_T,T)
                ###########################################################
                name = "aux_%s_epoch{.updater.epoch}" % (self.data[idx][0]) 
                name = name.format(trainer)
                visualize_alpha[name] = np.squeeze(att_w)
                ###########################################################
                att_w = self.get_attention_weight(idx, att_w)
            ...
            scio.savemat(self.outdir+'/attention_visual.mat', visualize_alpha)

        # elif type(att_ws) == tuple:
        #     [AM_att_ws, LM_att_ws] = att_ws
        #     for idx, att_w in enumerate(AM_att_ws): # for each sentence
        #         filename = "%s/%s.char.ep.{.updater.epoch}.png" % (self.outdir, self.data[idx][0]) # att_w is (dec_T,T)
        #         att_w = self.get_attention_weight(idx, att_w)
        #         self._plot_and_save_attention(att_w, filename.format(trainer))
        #     for idx, att_w in enumerate(LM_att_ws): # for each sentence
        #         filename = "%s/%s.bpe.ep.{.updater.epoch}.png" % (self.outdir, self.data[idx][0]) # att_w is (dec_T,T)
        #         att_w = self.get_attention_weight(idx, att_w)
        #         self._plot_and_save_attention(att_w, filename.format(trainer))
        else:
...
def mtlalpha_change(model, mtlalpha=0): # change one attr in model
    """Extension to reset mtlaplpha"""
    @training.make_extension(trigger=(1, 'epoch'))
    def mtlalpha_change(trainer):
        _mtlalpha_change(model, mtlalpha)
    return mtlalpha_change


def _mtlalpha_change(model, mtlalpha=0):
    if getattr(model, 'mtlalpha') != mtlalpha:
        setattr(model, 'mtlalpha', 0)
        logging.warning('mtlalpha is set to ' + str(mtlalpha))
    get_mtlalpha = getattr(model, 'mtlalpha')
    print('Setting mtlalpha is set to %2.1f' %get_mtlalpha )


class CompareValueTrigger_once(object):
    """Trigger invoked when key value getting bigger or lower than before
    ...
    def __call__(self, trainer):
        ...
        if not self._interval_trigger(trainer): # not the changing time(every batchsize)
            return False
        ...
        if self._compare_fn(self._best_value, value): # epoch 1 finished : this condition is OK.
            self._done = True
            return True
...
def schedule_sampling_increase(ssprob_list):
    """Extension to perform adadelta eps decay.
    ...
    """
    @training.make_extension(trigger=(1, 'epoch'))
    def schedule_sampling_increase(trainer):
        _schedule_sampling_increase(trainer, ssprob_list)

    return schedule_sampling_increase


def _schedule_sampling_increase(trainer, ssprob_list):
    optimizer = trainer.updater.get_optimizer('main')
    epoch_idx = optimizer.epochindex
    setattr(optimizer, 'epochindex', epoch_idx + 1)
    current_ssprobs = optimizer.sampling_probability
    print('Setting schedule sampling probs from %f to %f' %(current_ssprobs, ssprob_list[epoch_idx + 1]))
    setattr(optimizer, 'sampling_probability', ssprob_list[epoch_idx + 1])
    logging.info('schedule sampling set to ' + str(optimizer.sampling_probability))



def schedule_sampling_increase(model, ssprob_list=[0.0]*20): # change one attr in model
    """Extension to reset mtlaplpha"""
    @training.make_extension(trigger=(1, 'epoch'))
    def schedule_sampling_increase(trainer):
        _schedule_sampling_increase(model, ssprob_list)
    return schedule_sampling_increase


def _schedule_sampling_increase(model, ssprob_list):
    epoch_current = getattr(model, 'epochindex')
    setattr(model, 'epochindex', epoch_current + 1)

    # print(ssprob_list)
    current_ssprobs = model.sampling_probability
    print('Setting schedule sampling probs from %f to %f while the epoch is %d' %(current_ssprobs, ssprob_list[epoch_current + 1], epoch_current+1))
    setattr(model, 'sampling_probability', ssprob_list[epoch_current + 1])
    model.sampling_probability = ssprob_list[epoch_current + 1]
    logging.info('schedule sampling set to ' + str(model.sampling_probability))
    print('----------------------------------------------------------------------')
    # exit(0)


################################################################
def mtlbeta_change(model, mtlbeta=0): # change one attr in model
    """Extension to reset mtlaplpha"""
    @training.make_extension(trigger=(1, 'epoch'))
    def mtlbeta_change(trainer):
        _mtlbeta_change(model, mtlbeta)
    return mtlbeta_change


def _mtlbeta_change(model, mtlbeta=0):
    if getattr(model, 'mtlbeta') != mtlbeta:
        setattr(model, 'mtlbeta', 0)
        logging.warning('mtlbeta is set to ' + str(mtlbeta))
    get_mtlbeta = getattr(model, 'mtlbeta')
    print('Setting mtlbeta is set to %2.1f' %get_mtlbeta )
################################################################