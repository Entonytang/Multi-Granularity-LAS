            ...
            for batch in it:
                ...
                with reporter_module.report_scope(observation):
                    ...
                    if self.ngpu == 0:
                        ...
                        DDP(self.model, x, range(self.ngpu))
...
class CustomUpdater(StandardUpdater):
    ...
    def __init__(self, model, grad_clip_threshold, train_iter,
                 optimizer, device, ngpu, grad_noise=False, accum_grad=1, use_apex=False, losstype=1):
    ...
    def update_core(self):
        ...
        if self.losstype == 1:
            if self.ngpu == 0:
                ...
                # loss = data_parallel(self.model, x, range(self.ngpu)).mean() / self.accum_grad # apex does not support torch.nn.DataParallel
                loss = DDP(self.model, x, range(self.ngpu)).mean() / self.accum_grad
                
        elif self.losstype == 2:
            loss = self.model(*x)[0]
            if type(loss) == list: # split time update
                [loss, aux_loss] = loss
                aux_loss.backward(retain_graph=True)  # Backprop
                aux_loss.detach()  # Truncate the graph
            if self.ngpu > 1: loss = loss.sum() / self.ngpu
        else:
            print('Error setting for losstype, please check it.')
            exit(0)
...
class CustomConverter(object):
    ...
    def __init__(self, subsampling_factor=1, dtype=torch.float32, multilabel=1):
        """Construct a CustomConverter object."""
        self.subsampling_factor = subsampling_factor
        self.ignore_id = -1
        self.dtype = dtype
        self.mode = 'doubley' if multilabel == 2 else 'singley'
    ...
    def __call__(self, batch, device=torch.device('cpu')):
        ...
        if self.mode == 'singley':        
        ...
        elif self.mode == 'doubley':
            xs, ys = batch[0] # batch-size
            ys1, ys2 = [],[]
            for y in ys: # for batchsize : [0,1,2,3,...,batch-1]
                [y_task1, y_task2] = y
                ys1.append( y_task1 )
                ys2.append( y_task2 )
            ...
            ilens = torch.from_numpy(ilens).to(device)
            ys1_pad = pad_list([torch.from_numpy(y).long() for y in ys1], self.ignore_id).to(device) # torch.Tensor
            ys2_pad = pad_list([torch.from_numpy(y).long() for y in ys2], self.ignore_id).to(device) # torch.Tensor
            return xs_pad, ilens, [ys1_pad, ys2_pad]
...
def train(args):
    ...
    elif args.multi_label == 2:
        with open(args.valid_json, 'rb') as f:
            valid_json = json.load(f)['utts']
        utts = list(valid_json.keys())
        idim_list = [int(valid_json[utts[0]]['input'][i]['shape'][-1]) for i in range(args.num_encs)]
        odim_char = int(valid_json[utts[0]]['output'][0]['shape'][1])
        odim_bpe  = int(valid_json[utts[0]]['output'][0]['shape_bpe'][1])
        for i in range(args.num_encs):
            logging.info('stream{}: input dims : {}'.format(i + 1, idim_list[i]))
        logging.info('# (CHAR) output dims: ' + str(odim_char))        
        logging.info('# (BPE)  output dims: ' + str(odim_bpe))        
        odim = [odim_char, odim_bpe]
    else:
        print("Error setting at asr.py")
        exit(0)
    ...
    if (args.enc_init is not None or args.dec_init is not None) and args.num_encs == 1:
        ...
        model = model_class(idim_list[0] if args.num_encs == 1 else idim_list, odim, args) # TJ:2019.12.30 create model
    assert isinstance(model, ASRInterface)

    ###### added here start ######
    import pprint
    params = list(model.named_parameters())
    print("Model Params:\n{0}".format(pprint.pformat(sorted([p[0]+'.shape :'+str((p[1].detach().numpy()).shape)+'.number :'+str((p[1].detach().numpy()).size) for p in params]))))
    tot_size = 0
    for param in params: # (str, torch.nn.parameter.Parameter') -> 
        tot_size += param[1].size().numel()
    logging.warning("The Number of Model Parameters: {:.2f}M".format(tot_size*1.0/1024/1024)) 
    print('Optimization & Training') 
    ##############################

    ############################## TJ-SS Start
    if args.sampling_probability > 0:
        # print('Setting adjust schedule sampling with probability %3.1f' %(args.sampling_probability))
        setting_ss = args.sampling_probability 
        args.sampling_probability = 0.0
    else:
        setting_ss = 0.0
    ############################## TJ-SS End
    ...
    # Setup a converter
    if args.num_encs == 1:
        converter = CustomConverter(subsampling_factor=model.subsample[0], dtype=dtype, multilabel=args.multi_label)
    else:
    ...
    Lmode = 'mlt-asr' if args.multi_label == 2 else 'asr'
    load_tr = LoadInputsAndTargets(
        mode=Lmode, load_output=True, preprocess_conf=args.preprocess_conf,
    ...
    updater = CustomUpdater(
        ...
        device, args.ngpu, args.grad_noise, args.accum_grad, use_apex=use_apex, losstype=args.multi_label)
    ...
    if args.num_save_attention > 0 and args.mtlalpha != 1.0:
        ...
        if hasattr(model, "module"):
            att_vis_fn = model.module.calculate_all_attentions # TJ:2019.12.30 evaluate eval FACC
    ...
    if args.multi_label == 1:
        trainer.extend(extensions.PlotReport(['main/penalty', 'validation/main/penalty'], 'epoch', file_name='penalty.png'))        
        trainer.extend(extensions.PlotReport(['main/loss', 'validation/main/loss',
                                              'main/loss_ctc', 'validation/main/loss_ctc',
                                              'main/loss_att', 'validation/main/loss_att'] + ([] if args.num_encs == 1 else report_keys_loss_ctc), 'epoch', file_name='loss.png'))
        trainer.extend(extensions.PlotReport(['main/acc', 'validation/main/acc'], 'epoch', file_name='acc.png'))
        trainer.extend(extensions.PlotReport(['main/cer_ctc', 'validation/main/cer_ctc'] + ([] if args.num_encs == 1 else report_keys_loss_ctc), 'epoch', file_name='cer.png'))
        if args.model_module == "espnet.nets.pytorch_backend.e2e_asrA:E2EA":
            trainer.extend(extensions.PlotReport(['main/alpe_bpe', 'validation/main/alpe_bpe'],  'epoch', file_name='alpe_bpe.png'))
    elif args.multi_label == 2:
        trainer.extend(extensions.PlotReport(['main/mtl_loss',      'validation/main/mtl_loss',
                                              'main/loss_ctc',      'validation/main/loss_ctc',
                                              'main/loss_att_char', 'validation/main/loss_att_char',
                                              'main/loss_att_bpe',  'validation/main/loss_att_bpe'], 'epoch', file_name='loss.png'))
        trainer.extend(extensions.PlotReport(['main/acc', 'validation/main/acc'], 'epoch', file_name='acc_char.png'))
        trainer.extend(extensions.PlotReport(['main/acc_bpe',  'validation/main/acc_bpe'],  'epoch', file_name='acc_bpe.png'))        
        trainer.extend(extensions.PlotReport(['main/cer_ctc',  'validation/main/cer_ctc'],  'epoch', file_name='cer.png'))
        ##############################################################################################################################################
        if args.model_module == "espnet.nets.pytorch_backend.e2e_asrsA:E2EsA":
            trainer.extend(extensions.PlotReport(['main/acc_aux', 'validation/main/acc_aux'],  'epoch', file_name='acc_aux.png'))
        elif args.model_module == "espnet.nets.pytorch_backend.e2e_asrsA2:E2EsA2":
            trainer.extend(extensions.PlotReport(['main/acc_aux', 'validation/main/acc_aux'],  'epoch', file_name='acc_aux.png'))
            trainer.extend(extensions.PlotReport(['main/alpe_aux', 'validation/main/alpe_aux'],  'epoch', file_name='alpe_aux.png'))
            trainer.extend(extensions.PlotReport(['main/alpe_bpe', 'validation/main/alpe_bpe'],  'epoch', file_name='alpe_bpe.png'))
        elif args.model_module == "espnet.nets.pytorch_backend.e2e_asrsA3:E2EsA3":
            trainer.extend(extensions.PlotReport(['main/acc_char', 'validation/main/acc_char'],  'epoch', file_name='acc_char.png'))
            trainer.extend(extensions.PlotReport(['main/alpe_inter', 'validation/main/alpe_inter'],  'epoch', file_name='alpe_inter.png'))
            trainer.extend(extensions.PlotReport(['main/alpe_bpe', 'validation/main/alpe_bpe'],  'epoch', file_name='alpe_bpe.png'))         
        ##############################################################################################################################################

    if args.mtlalpha > 0 and args.adjust_closectc_weight > 0:
        from espnet.asr.asr_utils import CompareValueTrigger_once, mtlalpha_change
        print( 'Warning(CTC-CLOSED) : The CTC loss will closed after having Val-FACC larger than %f' %(args.closethreshold) ) ##
        trainer.extend(mtlalpha_change(model), trigger=CompareValueTrigger_once('validation/main/acc', lambda best_value, current_value: current_value > args.closethreshold) )
    if args.mtlbeta > 0 and args.adjust_closectc_weight > 0: ## New Add
        from espnet.asr.asr_utils import CompareValueTrigger_once, mtlbeta_change
        print("Warning : the Beta auxilitary loss will closed when FACC > %3.2f" %(args.closebeta_vals))
        trainer.extend(mtlbeta_change(model), trigger=CompareValueTrigger_once('validation/main/acc', lambda best_value, current_value: current_value > args.closebeta_vals) )

    ############################## TJ-SS Start
    if setting_ss > 0:
        from espnet.asr.asr_utils import schedule_sampling_increase
        trainer.extend(schedule_sampling_increase(model, args.ssprobs), trigger=(1, 'epoch') ) # setting_penalty_weight
        args.sampling_probability = setting_ss
    ############################## TJ-SS End
    ...
    if args.multi_label == 1:
        report_keys = ['epoch', 'iteration', 
                        'main/loss',            'main/loss_ctc',            'main/loss_att',            'main/penalty', 
                        'validation/main/loss', 'validation/main/loss_ctc', 'validation/main/loss_att', 'validation/main/penalty', 
        ...
        if args.model_module == "espnet.nets.pytorch_backend.e2e_asrA:E2EA":
            report_keys = report_keys + ['main/alpe_bpe', 'validation/main/alpe_bpe']
    elif args.multi_label == 2:
        report_keys = ['epoch', 'iteration', 
                       'main/loss'           , 'main/loss_ctc'           , 'main/loss_att_char'           , 'main/loss_att_bpe'           ,
                       'validation/main/loss', 'validation/main/loss_ctc', 'validation/main/loss_att_char', 'validation/main/loss_att_bpe',
                       'main/acc',     'validation/main/acc', 
                       'main/acc_bpe', 'validation/main/acc_bpe', 
                       'main/cer_ctc', 'validation/main/cer_ctc',
                       'elapsed_time']
        ##############################################################################################################################################
        if args.model_module == "espnet.nets.pytorch_backend.e2e_asrsA:E2EsA": report_keys = report_keys + ['main/acc_aux', 'validation/main/acc_aux']
        if args.model_module == "espnet.nets.pytorch_backend.e2e_asrsA2:E2EsA2": 
            report_keys = report_keys + ['main/acc_aux', 'validation/main/acc_aux']
            report_keys = report_keys + ['main/alpe_aux', 'validation/main/alpe_aux']
            report_keys = report_keys + ['main/alpe_bpe', 'validation/main/alpe_bpe']
        elif args.model_module == "espnet.nets.pytorch_backend.e2e_asrsA3:E2EsA3": 
            report_keys = report_keys + ['main/acc_char', 'validation/main/acc_char']
            report_keys = report_keys + ['main/alpe_inter', 'validation/main/alpe_inter']
            report_keys = report_keys + ['main/alpe_bpe', 'validation/main/alpe_bpe']         
        ##############################################################################################################################################
...
def recog(args):
    ...
    ########################################
    if args.dec_level == "BPE":
        with open(train_args.dict, 'rb') as f:
            dictionary = f.readlines() 
    elif args.dec_level == "CHAR":
        with open(train_args.chardict, 'rb') as f:
            dictionary = f.readlines()
    else:
        print('ERROR Decoder Setting.')
        exit(0)
    ...
    train_args.char_list = char_list
    ########################################
