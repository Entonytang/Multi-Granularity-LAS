...
def train(args):
    ...
    if args.mtlalpha > 0 and args.adjust_closectc_weight > 0:
        ...
        trainer.extend(mtlalpha_change(model), trigger=CompareValueTrigger_once('validation/main/acc', lambda best_value, current_value: current_value > 0.80) )        
...
def recog(args):
    ...
    ########################################
    if args.decode_choice == 'bpe' and train_args.multi_label == 2:
        with open(args.bpedict, 'rb') as f:
            dictionary = f.readlines()
        bpe_list = [entry.decode('utf-8').split(' ')[0]
                     for entry in dictionary]
        bpe_list.insert(0, '<blank>')
        bpe_list.append('<eos>')
        train_args.bpe_list = bpe_list
    elif args.decode_choice == 'char' and train_args.multi_label == 2:
        with open(args.chardict, 'rb') as f:
            dictionary = f.readlines()
        ...
        with torch.no_grad():
            for names in grouper(args.batchsize, keys, None):
                ...
                if args.streaming_mode == 'window' and args.num_encs == 1:
                    ...
                    if args.decode_choice == 'bpe' and train_args.multi_label == 2:
                        nbest_hyps = model.recognize_batch(feats, args, train_args.bpe_list, rnnlm=rnnlm)
                    elif args.decode_choice == 'char' and train_args.multi_label == 2:
                ...
                for i, nbest_hyp in enumerate(nbest_hyps):
                    name = names[i]
                    if args.decode_choice == 'bpe' and train_args.multi_label == 2:
                        new_js[name] = add_results_to_json(js[name], nbest_hyp, train_args.bpe_list)
                    elif args.decode_choice == 'char' and train_args.multi_label == 2:
