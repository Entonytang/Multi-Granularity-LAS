...
def get_parser(parser=None, required=True):
    ...
    parser.add_argument('--dict', required=required,
                        help='Dictionary')
    parser.add_argument('--chardict', type=str, help='CHAR Dictionary : for inverse BPE to CHAR.') ## added by jtang ##
    ...
    parser.add_argument('--mtlbeta', default=0.00, type=float,
                        help='Multitask learning coefficient, beta ')
    ...
    parser.add_argument('--fbank-fmax', type=float, default=None,
                        help='')
    # I/O interface 
    parser.add_argument('--multi-label', default=1, type=int,
                        help='Using multi level output token (1 is default, 2 is for BPE&CHAR.)')
    parser.add_argument('--model_module', type=str, default=None, choices=['Stand', 'StandA', 'Multi', 'MultiM', 'MultiSS', 'MultiAUX', 'MultiAUX2', 'MultiAUX3'],
                        help='Stand is e2e_asr:E2E / Multi is e2e_asr:E2E / MultiM is e2e_asr:E2Em')
    parser.add_argument('--adjust-closectc-weight', default=0.0, type=float,
                        help='Close CTC weight')    
    parser.add_argument('--closethreshold', type=float, default=0.50, 
                        help='The Val-FACC threshold for closing CTC loss.')
    parser.add_argument('--closebeta_vals', type=float, default=0.82, 
                        help='The Val-FACC threshold for closing AUX loss.')    
    parser.add_argument('--divpenaltyweight', default=0.0, type=float,
                        help='The divergency penalty weight for posterior.')     

    parser.add_argument('--epochindex', default=0, type=int, help='The index of epochs.')
    # parser.add_argument('--ssprobs', default=[0, 0, 0, 0, 0, 0,
    #                                              0.1, 0.2, 0.3, 0.4, 
    #                                              0.4, 0.4, 0.4, 0.4,
    #                                              0.4, 0.4, 0.4, 0.4,
    #                                              0.4, 0.4, 0.4, 0.4
    #                                              ], type=list, help='The index of epochs.')
    ...
    parser.add_argument('--ssprobs', default=[0, 0, 0, 0, 0, 0,
                                                 ...
                                                 0.40, 0.40, 0.40, 0.40,
                                                 0.40, 0.40, 0.40, 0.40
                                                 ], type=list, help='The index of epochs.') ## ss-v4    
    return parser
...
def main(cmd_args):
    ...
    if args.model_module == "Stand":
        print('Enter the standard I/O interface : 1 input with 1 output')
        model_module = "espnet.nets." + args.backend + "_backend.e2e_asr:E2E"

    elif args.model_module == "StandA":
        print('Enter the standard I/O interface : 1 input with 1 output [StandA]')
        model_module = "espnet.nets." + args.backend + "_backend.e2e_asrA:E2EA"

    elif args.model_module == "Multi": ## added by jtang ##
        print('Enter the MultiTask I/O interface : 1 input with 2 outputs')
        model_module = "espnet.nets." + args.backend + "_backend.e2e_asrs:E2Es"

    elif args.model_module == "MultiSS": ## added by jtang ##
        print('Enter the MultiTask I/O interface : 1 input with 2 outputs + Schedule Sampling Operation')
        model_module = "espnet.nets." + args.backend + "_backend.e2e_asrss:E2Eschedules"

    elif args.model_module == "MultiAUX": ## added by jtang ##
        print('Enter the MultiTask I/O interface (AUX) : 1 input with 2 outputs')
        model_module = "espnet.nets." + args.backend + "_backend.e2e_asrsA:E2EsA"

    elif args.model_module == "MultiAUX2": ## added by jtang ##
        print('Enter the MultiTask I/O interface (AUX2) : 1 input with 2 outputs')
        model_module = "espnet.nets." + args.backend + "_backend.e2e_asrsA2:E2EsA2"

    elif args.model_module == "MultiAUX3": ## added by jtang ##
        print('Enter the MultiTask I/O interface (AUX3) : 1 input with 2 outputs')
        model_module = "espnet.nets." + args.backend + "_backend.e2e_asrsA3:E2EsA3"        

    elif args.model_module == "MultiM": ## added by jtang ##
        print('Enter the MultiTask I/O interface : 1 input with 2 outputs (share part encoder, split 1 layer)')
        model_module = "espnet.nets." + args.backend + "_backend.e2e_asrsm:E2Esm"        
    else:
    ...
    if args.dict is not None:
        ...
        bpe_list.append('<eos>')
        args.bpe_list = bpe_list
    else:
        args.bpe_list = None

    # load dictionary for debug log (bpe) ## added by jtang ##
    if args.chardict is not None:
        if args.model_module == "Multi" or args.model_module == "MultiM" or args.model_module == "MultiAUX":
